...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // customer last answer

        View::composer(['theme::layouts.main', 'theme::page'], function ($view) {
            if (Auth::check()) {
                $customerId = Auth::user()->id;

                $customer_give_question_obj_get_all_ans = StylistCustomerQuestionsAnswer::where('customer_id', $customerId)->orderby('id', 'desc')->pluck('answer_ids')->first();

                if ($customer_give_question_obj_get_all_ans)
                {
                    $view->with('customer_give_question_obj_get_all_ans', $customer_give_question_obj_get_all_ans);
                    $view->with('customer', $customerId);
        ...
        if (
            isset($_SERVER['HTTP']) &&
            ($_SERVER['HTTP'] == 'on' || $_SERVER['HTTP'] == 1) ||
