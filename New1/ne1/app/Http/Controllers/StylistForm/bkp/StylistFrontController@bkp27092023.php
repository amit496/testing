            ...
            elseif(isset($stylist_call_complete))
            {
                ...
                if(isset($booking_obj))
                {
                    ...
                    if($reveal_info_has->isNotEmpty())
                    {
                        ...
                        else if(isset($booking_obj) && ($booking_obj->status =='return_initiated') && ($reveal_info_has_info ->status =='return_initiated') && isset($order_data_info) && (($order_data_info->order_status_id == 6) || ($order_data_info->order_status_id != 6)) && isset($cancel_data) && (($cancel_data->status == 6)) && (strtotime($cancel_data->created_at) <= strtotime($update_14_days)) && (strtotime($today) >= strtotime($reveal_create_76_days) || strtotime($today) <= strtotime($reveal_create_76_days))  && (strtotime($today) <= strtotime($reveal_create_45_days)))
                            {
                                $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name .'!</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">Reveal Coming Soon</span>';
                                $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Sit tight, you can expect your reveal in about 2 months.</h6></div>';
                                $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">REVEAL COMING SOON</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/women_clothing_style_and_fit_section_image.webp') . ');"></div></div>';
                            }
                        else if(isset($order_data_info)  && ($booking_obj->status =='return_initiated') && ($reveal_info_has_info->status =='return_initiated') && (($order_data_info->order_status_id == 6) || ($order_data_info->order_status_id != 6)) && isset($cancel_data) && ($cancel_data->status == 1) && (strtotime($cancel_data->created_at) <= strtotime($update_14_days)))
                        {
                        ...
                        else if(($reveal_info_has_info->status != 'completed') && (($reveal_info_has_info->status != 'draft')) || ($reveal_info_has_info->status == 'in_progress'))
                        {
        ...
        if($orders->count() > 0)
        {
            foreach ($orders as $key => $order)
            {
                foreach ($order->inventories as $item)
                {
                    ...
                    $buy_html .= '<div class="col-md-4  img-product-1 " style="'.$cancel_item_hide.'"><div class="rounded">'.$product_image_html.'</div><div class="articles-two"><h6>'.$product_brand.'</h6><h6>'.$product_name.'</h6></div></div>';
                }
                // print_r($order->reveal_products_ids);
