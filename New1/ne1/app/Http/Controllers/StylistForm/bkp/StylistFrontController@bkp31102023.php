...
class StylistFrontController extends Controller
{
    ...
    ->with([
        'shop:id,name,slug',
        'inventories:id,title,slug,product_id,description,brand',
        'inventories.images:id,path,imageable_id,imageable_type', // Use 'images' as the relationship name.
        'inventories.product.image:path,imageable_id,imageable_type',
        'cancellation',
    ])
    ->orderBy('id', 'desc')
        ...
        if($orders->count() > 0)
        {
            foreach ($orders as $key => $order)
            {
                foreach ($order->inventories as $item)
                {
                    ...
                    // $get_inventry_image = $item->image::where('imageable_id',$item->id)->latest();
                    // $get_product_image = $item->images()->where('imageable_id',$item->product_id)->first();
                    // $get_inventory_image = $item->images()->where('imageable_id', $item->id)->first();
                    // if ($get_inventory_image)
                    // {
                    //     if ($item->id  == optional($get_product_image)->imageable_id && optional($get_product_image)->imageable_type == 'App\Models\Inventory')
                    //     {
                    //         $product_image_html = "<img src='".get_storage_file_newurl(optional($get_product_image)->path)."' alt='".$item->slug."' title='".$item->slug."' />";
                    //     }
                    //     echo $get_inventory_image->id;
                    // }


                    // if ($get_inventory_image) {
                    //     if ($get_inventory_image->imageable_id == $item->id && $get_inventory_image->imageable_type == 'App\Models\Inventory') {
                    //         $product_image_html = "<img src='" . get_storage_file_newurl($get_inventory_image->path) . "' alt='" . $item->slug . "' title='" . $item->slug . "' />";
                    //     }
                    //     // echo $get_inventory_image->id;
                    // }
                    // else if($get_product_image)
                    // {
                    //     if($get_product_image->imageable_id == $item->product_id && $get_product_image->imageable_type == 'App\Models\Product')
                    //     {
                    //         $product_image_html = "<img src='".get_storage_file_newurl($get_product_image->path)."' alt='".$item->slug."' title='".$item->slug."' />";
                    //     }
                    // }


                    // if($get_inventry_image != '')
                    // {
                    //     foreach($get_inventry_image as $key => $get_inventry_val)
                    //     {
                    //         echo $get_inventry_val->id;
                    //     }
                    // }
                    ...
                    if(isset($order->cancellation))
                    {
                        ...
                        if(array_key_exists($item->id, $json_encode_exchange_option) && ($json_encode_exchange_option[$item->id] == 'exchange')  && in_array($item->id,$json_encode_declienids ))
                        {
                            $forced_hide_items = 'display:none;';
                        }
                        else if(array_key_exists($item->id, $json_encode_exchange_option) && ($json_encode_exchange_option[$item->id] == 'exchange')  && in_array($item->id,$json_encode_approvedids ))
                        {
                    ...
                    $buy_html .= '<div class="col-md-4  img-product-1 " style="'.$forced_hide_items.'"><div class="rounded">'.$product_image_html.'</div><div class="articles-two"><h6>'.$product_brand.'</h6><h6>'.$product_name.'</h6></div></div>';
                }
