...
use App\Models\FilterRevealStatus;
...
class StylistFrontController extends Controller
{
    ...
    public function saveBooking($stylist_id = 0, $booking_date = '', $booking_time = '')
    {
        ...
        // if (isset($records_obj))
        // {
        //     $new_booking_data = StylistClientBookingAppointments::where('customer_id', $customer_id)->first();
        //     echo 1111;
        //     if(isset($new_booking_data))
        //     {
        //         $new_filter_data = new FilterRevealStatus;
        //         $new_filter_data->customer_id = $customer_id;
        //         $new_filter_data->customer_name = $customer_name;
        //         $new_filter_data->merchant_id = $stylist_id;
        //         // $new_filter_data->reveal_id = '';
        //         $new_filter_data->booking_id = $new_booking_data->id;
        //         $new_filter_data->status = 'not_started';
        //     }
        // }
    ...
    public function customerBookingReviewhtml()
    {
        ...
        if($data_obj)
        {
            ...
            {
                if($reveal_info_draft_has->isNotEmpty())
                {
                    foreach($reveal_info_draft_has as $key => $reveal_info_draft_has_info)
                    {
                        $reveal_status_info = $reveal_info_draft_has_info->status;
                        array_push($reveal_status_info_arr, $reveal_status_info);
                        // print_r($reveal_status_info_arr);
                        ...
                        // echo "<pre>";
                        // print_r($search_key);
                        if(in_array('draft', $reveal_status_info_arr) && !in_array('awaiting_response', $reveal_status_info_arr))
                        {
                            $reveal_show = 'display:block !important; margin:  0 auto !important';
                        }
                        else if(in_array('draft', $reveal_status_info_arr) && in_array('awaiting_response', $reveal_status_info_arr))
                        {
                            $reveal_show = 'display:none !important; margin: 0 auto !important ';
                        }
                        else
                        {
                            $reveal_show = '';
                        }
                        // foreach($array_countvalues as $key => $array_countvalues_info)
                        // {

                        //     // echo $key . ' = ' .$array_countvalues_info;

                        //     if ( (count($reveal_status_info_arr) == 2 ) && (($reveal_status_info == 'draft') && ($key == 'draft') && ($array_countvalues_info == 1) && ($search_key == 1)) && (($reveal_status_info == 'awaiting_response') && ($key == 'awaiting_response') && ($array_countvalues_info == 1) && ($search_key == 1)))
                        //     {
                        //         // $reveal_hide = '';
                        //         echo 1;
                        //         $reveal_show = 'display:block';
                        //     }
                        //     else if( (count($reveal_status_info_arr) == 1 ) && ($reveal_status_info == 'draft') && ($key == 'draft')  && ($array_countvalues_info == 1) && ($search_key == 1))
                        //     {
                        //         echo 2;
                        //         $reveal_show = 'display:block';
                        //     }
                        //     else if((count($reveal_status_info_arr) == 2 ) && ($reveal_status_info == 'draft') && ($key == 'draft') && ($array_countvalues_info == 2) && ($search_key == 2))
                        //     {
                        //         echo 3;
                        //         $reveal_show = 'display:block';
                        //     }
                ...
                $booking_obj = StylistClientBookingAppointments::where('customer_id', $customer_id)->latest()->first();
                ...
                if(isset($booking_obj))
                {
                    ...
                    $reveal_info_has = stylistRevealsItems::where('booking_id', $booking_obj->id)->latest()->take(2)->get();
                    $order_data_info ='';
                    $update_days_diff   = '';
                    $cancel_data = '';
                    if($reveal_info_has->isNotEmpty())
                    {
                        foreach($reveal_info_has as $key => $reveal_info_has_info)
                        {
                            $reveal_status_info = $reveal_info_has_info->status;
                            array_push($reveal_status_info_arr, $reveal_status_info);
                            // print_r($reveal_status_info_arr);
                            $array_countvalues= array_count_values($reveal_status_info_arr);
                            $search_key = array_key_exists($reveal_status_info, $array_countvalues);
                            ...
                            else if(in_array('draft', $reveal_status_info_arr) && (in_array('in_progress', $reveal_status_info_arr)))
                            {
                                // echo 3;
                                $reveal_show = 'display:block !important; margin: 0 auto !important ';
                            }
                        ...
                        $order_data_info = Order::where('customer_id', $customer_id)->where('reveal_id', $reveal_info_has_info->id)->latest()->first();
                        $today = date('d-m-Y');
                        if($order_data_info)
                        {
                            $now = time();
                        ...
                        $reveal_id = $reveal_info_has_info->id;
                        $reveal_create_date  = $reveal_info_has_info->created_at->format('d-m-Y');
                        ...
                        $reveal_name = $reveal_info_has_info->name;
                        $updated_at = $reveal_info_has_info->updated_at;
                        ...
                        if((strtotime($today) >= strtotime($reveal_create_14_days)) && (strtotime($today) >= strtotime($reveal_create_date_3month)))
                        {
                        ...
                        else if(isset($booking_obj) && ($booking_obj->status =='dispatched') && ($reveal_info_has_info->status =='dispatched') && isset($order_data_info) && ($order_data_info->order_status_id == 6) && !isset($cancel_data) && (strtotime($today) >= strtotime($update_14_days)) && ((strtotime($today) >= strtotime($reveal_create_76_days)))  && (strtotime($today) <= strtotime($reveal_create_45_days)))
                        {
                        ...
                        else if(isset($booking_obj) && ($booking_obj->status =='return_initiated') && ($reveal_info_has_info->status =='refunded') && isset($order_data_info) && (($order_data_info->order_status_id == 7)) && isset($cancel_data) && (($cancel_data->status == 6)) && (strtotime($cancel_data->created_at) <= strtotime($update_14_days)) && (strtotime($today) >= strtotime($reveal_create_76_days) || strtotime($today) <= strtotime($reveal_create_76_days))  && (strtotime($today) <= strtotime($reveal_create_45_days)))
                        {
                            $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name .'!</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">Reveal Coming Soon</span>';
                            $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Sit tight, you can expect your reveal in about 2 months.</h6></div>';
                            $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">REVEAL COMING SOON</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/women_clothing_style_and_fit_section_image.webp') . ');"></div></div>';
                        }
                        else if(isset($booking_obj) && ($booking_obj->status =='return_initiated') && ($reveal_info_has_info ->status =='return_initiated') && isset($order_data_info) && (($order_data_info->order_status_id == 6)) && isset($cancel_data) && (($cancel_data->status == 6)) && (strtotime($cancel_data->created_at) <= strtotime($update_14_days)) && (strtotime($today) >= strtotime($reveal_create_76_days) || strtotime($today) <= strtotime($reveal_create_76_days))  && (strtotime($today) <= strtotime($reveal_create_45_days)))
                            {
                                $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name .'!</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">Reveal Coming Soon</span>';
                                $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Sit tight, you can expect your reveal in about 2 months.</h6></div>';
                                $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">REVEAL COMING SOON</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/women_clothing_style_and_fit_section_image.webp') . ');"></div></div>';
                            }
                        else if(isset($order_data_info)  && ($booking_obj->status =='return_initiated') && ($reveal_info_has_info->status =='return_initiated') && (($order_data_info->order_status_id == 6)) && isset($cancel_data) && ($cancel_data->status == 1) && (strtotime($cancel_data->created_at) <= strtotime($update_14_days)))
                        {
                            $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name . '!</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">Return Being Process</span>';
                            ...
                            $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;" style="max-width: 300px;">Return Being Process</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/Dappr_smart_casual.jpg') . ');"></div></div>';
                        }
                        else if(isset($order_data_info) && ($order_data_info->order_status_id == 6) && ($booking_obj->status =='dispatched') && ($reveal_info_has_info->status =='delivered') && (strtotime($today) <= strtotime($update_14_days)))
                        {
                        ...
                        else if(isset($order_data_info) && ($order_data_info->order_status_id == 5) && ($booking_obj->status =='dispatched') && ($reveal_info_has_info->status =='awaiting_delivery') && (strtotime($today) <= strtotime($update_14_days)))
                        {
                            $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name . '!</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">Order Confirmed </span>';
                            ...
                            $html_booking_info .= '
                            <div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">Order Confirmed</a>
                            </div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/women_colour_and_fabric_section_image.webp') . ');"></div></div>';
                        }
                        else if(isset($order_data_info) && (($order_data_info->order_status_id == 1) || ($order_data_info->order_status_id == 3) || ($order_data_info->order_status_id == 4)) &&(($reveal_info_has_info->status == 'preparing_order') || ($reveal_info_has_info->status == 'dispatched')) && (($booking_obj->status =='preparing_order') || ($booking_obj->status =='dispatched')) && (strtotime($today) <= strtotime($update_14_days)))
                        {
                        ...
                        else if(isset($order_data_info) && (($order_data_info->order_status_id == 1) || ($order_data_info->order_status_id == 3) || ($order_data_info->order_status_id == 4)) &&(($reveal_info_has_info->status == 'preparing_order')) && (($booking_obj->status =='preparing_order')) )
                        {
                            $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name . '!</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">Items Coming</span>';
                            $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Check your order for status updates.</h6></div>';
                        ...
                        else if(($reveal_info_has_info->status != 'completed') && (($reveal_info_has_info->status != 'draft')) || ($reveal_info_has_info->status == 'in_progress') && ($reveal_info_has_info->status != 'return_initiated'))
                        {
    ...
    public function getCustomerReviewProductsDetails()
    {
        ...
        if($orders->count() > 0)
        {
            foreach ($orders as $key => $order)
            {
                ...
                {
                    $inventory_pro_id = $item->product_id;
                    $item_data = explode(',',$item->pivot->item_description);
                    foreach($item_data as $key=> $item_data_val)
                    {
                        $product_brand = isset($item_data[0])? $item_data[0]:' ';
                        $product_name = isset($item_data[1])? $item_data[1]:' ';
                    }
                    $product_image_html = " ";
                    if ($item->id  == optional($item->image)->imageable_id && optional($item->image)->imageable_type == 'App\Models\Inventory')
                    {
                        $product_image_html = "<img src='".get_storage_file_newurl(optional($item->image)->path)."' alt='".$item->slug."' title='".$item->slug."' />";
                    }
                    ...
                    {
                        $product_image_html = "<img src='".get_storage_file_newurl(optional($item->product->image)->path)."' alt='".$item->slug."' title='".$item->slug."' />";
                    }
                    else
                    {}
                    ...
                    {
                        $exchange_option = $order->cancellation->exchnage_return_option;
                        $approvedids = $order->cancellation->approve_items;
                        $declienids = $order->cancellation->decline_item;
                        $json_encode_exchange_option = json_decode($exchange_option, true);
                        $json_encode_approvedids = explode(',',$approvedids);
                        $json_encode_declienids = explode(',',$declienids);

                        if(array_key_exists($item->id, $json_encode_exchange_option) && $json_encode_exchange_option[$item->id] == 'exchange')
                        {
                            $forced_hide_items = 'display:none;';
                        }
                        else if(array_key_exists($item->id, $json_encode_exchange_option) && ($json_encode_exchange_option[$item->id] == 'refund') && in_array($item->id,$json_encode_approvedids ))
                        {
                            $forced_hide_items = 'display:none;';
                        }
                        else
                        {
                            $forced_hide_items = 'display:block;';
                        }
                    ...
                    $buy_html .= '<div class="col-md-4  img-product-1 " style="'.$forced_hide_items.'"><div class="rounded">'.$product_image_html.'</div><div class="articles-two"><h6>'.$product_brand.'</h6><h6>'.$product_name.'</h6><h6>'.$item->id.'</h6></div></div>';
                }
                $buy_html .= '<div class="paginate_num">'.$orders->links().'</div>';
            }
        }
        // dd($orders);
        // echo $orders->links();
