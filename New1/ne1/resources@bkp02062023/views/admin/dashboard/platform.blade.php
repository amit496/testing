        ...
        <div class="row box" style="margin-left: 0px;">
          <div class="col-lg-12 p-1">
            ...
            <table class="table">
              ...
              <tbody>
                @foreach ($order_data as $key=> $order_data_val )
                  <tr>
                    <td><a href="{{ route('admin.order.order.show', $order_data_val->id) }}">
                      {{ $order_data_val->order_number }}
                    </a></td>
                    <td>{{$order_data_val->customer->getName()}}</td>
                    <td>{{ get_formated_currency($order_data_val->grand_total, 2) }}</td>
                    <td>{!! $order_data_val->paymentStatusName() !!}</td>
                    
                    <td>{{$order_data_val->created_at->format('d/m/Y')}}</td>
                  </tr>
                  {{-- {{$order_data_val->order_number}} --}}
                @endforeach
              </tbody>
        ...
        <div class="nav-tabs-custom">
          ...
          <div class="tab-content">
            <div class="tab-pane active" id="visitors_tab">
              @if (\App\Models\SystemConfig::isGgoogleAnalyticEnabled() && !\App\Models\SystemConfig::isGgoogleAnalyticConfigured())
