...
class CartController extends Controller
{
    ...
    public function remove(Request $request)
    {
        ...
        if ($item) {
                    // print_r(Session::get('stylist_reveal_product_order_list'));
                    $inv_product_id= Inventory::Where('id',$request->item)->first();
                    $stylist_form_id = Session::get('stylist_reveal_booking_id');
                        if($stylist_form_id != ''){
                            $stylist_form_id = array_keys($stylist_form_id);
                            $stylist_form_id = $stylist_form_id[0];
                            $selected_product_ids_arr = Session::get('stylist_reveal_product_order_list');
                            if($selected_product_ids_arr != '' && is_array($selected_product_ids_arr) && isset($selected_product_ids_arr[$stylist_form_id])){
                                $selected_product_ids_arr = $selected_product_ids_arr[$stylist_form_id];
                                $deleted_product_id = $inv_product_id->product_id;
                                if(in_array($deleted_product_id,$selected_product_ids_arr)){
                                    $delete_key =  array_search($deleted_product_id,$selected_product_ids_arr);
                                    if(isset($selected_product_ids_arr[$delete_key]))
                                    {
                                        unset($selected_product_ids_arr[$delete_key]);
                                        asort($selected_product_ids_arr);
                                        Session::put('stylist_reveal_product_order_list','');
                                        Session::put('stylist_reveal_product_order_list.'.$stylist_form_id, $selected_product_ids_arr);
                                        $StylistClientInfo_get_dta = StylistClientInfo::where('stylist_form_id',$stylist_form_id)->orderby('id','desc')->first();
                                        // dd($StylistClientInfo_get_dta);
                                        if(isset($StylistClientInfo_get_dta))
                                        {
                                            $StylistClientInfoDetails_data = StylistClientInfoDetails::where('stylist_info_id',$StylistClientInfo_get_dta->id)->get();
                                            if($StylistClientInfoDetails_data)
                                            {
                                                $update_data = array('product_id' => $deleted_product_id, 'stylist_info_id'=> $StylistClientInfo_get_dta->id);
                                                StylistClientInfoDetails::insert($update_data);
                                            }
                                        }
