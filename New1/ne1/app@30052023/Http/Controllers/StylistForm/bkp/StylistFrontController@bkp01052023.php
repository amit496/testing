                ...
                {   
                    ...
                    {   
                        ...
                        if($order_data_info)
                        {
                            ...
                            $update_14_days = date('d-m-Y', strtotime($order_data_info->updated_at. '+14days'));
                            $update_diff =strtotime($update_14_days) -  $now ;
                            $update_days_diff  =  round($update_diff / (60*60*24));
                            $cancel_data = cancellation::where('order_id', $order_data_info->id)->where('customer_id', $order_data_info->customer_id)->first();
                        } 
                    }
                    ...
                    {   
                        ...
                        if (isset($reveal_info_obj_new) && Carbon::parse($reveal_info_obj_new->updated_at)->diffInDays(Carbon::now()) >= $reveal_show_preview_day && Carbon::parse($reveal_info_obj_new->updated_at)->diffInDays(Carbon::now()) < $reveal_show_review_if_days_is_more_than && $reveal_info_obj_new->status != 'completed')
                        {
                            ...
                            $reveal_link = url("stylist/reveal/" . $booking_id . "/" . $reveal_id . '/' . $rand_code);
                            $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section">
                                                            <div class="reveal_booked_appointment_info_section_btn"><a href=' . $reveal_link . '>' . 'VIEW YOUR REVEAL' . '</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/') . '/Dappr_animal.jpg?' . rand(10, 1000) . ');"></div></div>';
                            $html_booking_info .= ' <div  class="btn stf_save_btn show_questions_screen_btn col-md-12 p-0"><a href=' . $reveal_link . ' class="btn text-light">' . 'VIEW REVEAL' . '</a></div> </div>';
                        }
                        else if( isset($order_data_info) && $order_data_info->order_status_id == 1 &&($reveal_info_has->status =='preparing_order') && $booking_obj->status =='preparing_order' )
                        {
                        ...
                        else if(isset($order_data_info) && $order_data_info->order_status_id == 6 && $booking_obj->status =='dispatched' && $reveal_info_has->status =='dispatched')
                        { 
                        ...
                        else if(isset($cancel_data) && $cancel_data->status == 1 && isset($order_data_info) && ($order_data_info->id == $cancel_data->order_id) && $cancel_data->created_at <= $update_14_days)
                        {
                        ...
                        else if(isset($cancel_data) && $cancel_data->status == 3 && isset($order_data_info) && ($order_data_info->id == $cancel_data->order_id) && $cancel_data->created_at <= $update_14_days)
                        {
                        ...
                        // New Reveal Upcoming 
                        else if($update_days_diff > 14  || !isset($cancel_data->order_id) || isset($order_data) && $order_data->order_status_id == 6)
                        {
                            $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name .'</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">Reveal coming soon</span>';
