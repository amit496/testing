...
class StylistFrontController extends Controller
{
        ...
        if ($booking_id != '') {
            ...
            if ($booking_info) {
                ...
                if (isset($reveal_info)) {
                    ...
                    $cart_details = DB::table('cart_items')->pluck('inventory_id')->toArray();
    ...
    {   
        ...
        if ($method_name == 'save_question_answers') {
            ...
            {
                $this->validate($request, [
                    'file' => 'required|mimes:png,jpeg,jpg,webp | max:20000',
                ]);
                $image_name = '';
            ...
            $output1  = array();
            //$output3  = array();
            if (is_array($data)) {$output1 = $this->questioninfo($data);}
    ...
    public function getBookingTimeList($stylist_id = 0, $booking_date = '')
    {
        ...
        // if(isNotEmpty())
        // {
        //     foreach($merchant_dates as $merchant_dates_info)
        //     {
        //        $merchant_dates_info->start_time;
        //        $merchant_dates_info->end_time;
        //     }
        // }
            ...
            // $times_am = array(9, 10, 11);
                ...
                //     $html .= '  <div class="style-field-checkbox-outer col-md-2_rename  style-field-checkbox-outer-box  " >
                //                 <div class=" product_box product_box_outer  ">
                //                 <h6 class="text-center mt-2  product_box_single_select  product_select_box  "> ' . $time_am_val . ' pm</h6>
                //                 <input type="radio" class="style-field-hide style-options-checkbox stylist_field_required" name="booking_time" value="' . $time_am_val . '">
                //                 </div>
                //         </div>';
                // }
    ...
    public function customerBookingReviewhtml()
    {
        ...
        $data_obj_revel ='';
        $data_obj = StylistClientBookingAppointments::where('customer_id', $customer_id)->where('status', '!=', $reveal_status)->first();
        if(isset( $data_obj ))
        {
            $data_obj_revel = stylistClientBookingAppointmentsChangeStatusHistory::where('booking_id', $data_obj->id)->where('status', '!=', 'sent')->where('reveal_id','==', 'null')->orWhere('status', '!=', 'call_complete')->first();
        ...
        if($data_obj)
        {
            ...
            $stylist_call_complete = $data_obj->statusHistory()->where('status', '=', 'call_complete')->first();
            ...
            elseif(isset($stylist_call_complete))
            {
                $html ='';
                $html_booking_info ='';
                $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto">
                ...
                $reveal_show_preview_day = stylistShowPreviousRevealBeforeDayHelper();
                $reveal_show_review_if_days_is_more_than = stylistShowSendMailNextRevealBeforeDayHelper();
                $order_reveal_ids = array();
                $order_details = Order::where('customer_id', $customer_id)->pluck('reveal_id')->toArray();
                if(isset($booking_obj))
                {
                    $booking_obj->created_at->format('d-m-Y');
                    $booking_obj_2_month = date('d-m-Y', strtotime($booking_obj->created_at .  "+2 months"));
                    $booking_obj_2_month_17_days = date('d-m-Y', strtotime($booking_obj_2_month .  "+17 days"));
                    $booking_obj_2_month_27_days = date('d-m-Y', strtotime($booking_obj_2_month_17_days .  "+10 days"));
                    $booking_obj_3_month = date('d-m-Y', strtotime($booking_obj->created_at .  "+3 months"));
                    $reveal_status = getRevealStatusKeyNameHelper('awaiting_response');
                    $reveal_info_has = stylistRevealsItems::where('booking_id', $booking_obj->id)->first();
                    $order_data_info ='';
                    $update_days_diff   = '';
                    $cancel_data = '';
                    if($reveal_info_has)
                    {   
                        $order_data_info = Order::where('customer_id', $customer_id)->where('reveal_id', $reveal_info_has->id)->latest()->first();
                        if($order_data_info)
                        {
                            $now = time();
                            $update_date = $order_data_info->updated_at;
                            $update_14_days = date('d-m-Y', strtotime($order_data_info->updated_at. '+14 days'));
                            $update_diff =strtotime($update_14_days) -  $now ;
                            $update_days_diff  =  round($update_diff / (60*60*24));
                            $cancel_data = cancellation::where('order_id', $order_data_info->id)->where('customer_id', $order_data_info->customer_id)->latest()->first();
                        } 
                        // $today = '11-08-2023';
                        ...
                        // print_r('booking_obj_2_month:- '. $booking_obj_2_month );
                        // echo "<br>";
                        // var_dump($booking_obj_2_month);
                        // echo "<br>";
                        // echo "<br>";

                        // print_r('$booking_obj_2_month_17_days:- '. $booking_obj_2_month_17_days);
                        // echo "<br>";
                        // var_dump($booking_obj_2_month_17_days);
                        // echo '<br>';
                        // echo "<br>";

                        // print_r('$booking_obj_2_month_27_days:- '. $booking_obj_2_month_27_days);
                        // echo "<br>";
                        // var_dump($booking_obj_2_month_27_days);
                        // echo '<br>';
                        // echo "<br>";

                        // print_r('$booking_obj_3_month:- '. $booking_obj_3_month);
                        // echo "<br>";
                        // var_dump($booking_obj_3_month);
                        // echo '<br>';
                        // echo "<br>";


                        // if(strtotime($today) >= strtotime($booking_obj_2_month) && (strtotime($today) < strtotime($booking_obj_2_month_17_days)) && (strtotime($today) < strtotime($booking_obj_2_month_27_days)) && (strtotime($today) < strtotime($booking_obj_3_month)))
                        // {
                        //     echo '1 Month';
                        // }
                        // else if(strtotime($today) > strtotime($booking_obj_2_month) && (strtotime($today) >= strtotime($booking_obj_2_month_17_days)) && (strtotime($today) < strtotime($booking_obj_2_month_27_days)) && (strtotime($today) < strtotime($booking_obj_3_month)))
                        // {
                        //     echo '10 days';
                        // }
                        // else if(strtotime($today) > strtotime($booking_obj_2_month) && (strtotime($today) > strtotime($booking_obj_2_month_17_days)) && (strtotime($today) >= strtotime($booking_obj_2_month_27_days)) && (strtotime($today) < strtotime($booking_obj_3_month)))
                    ...
                    if(isset($reveal_info_has) && ($reveal_info_has->status != 'completed') )
                    {   
                        $today = date('d-m-Y');
                        $reveal_id = $reveal_info_has->id;
                        $reveal_create_date  = $reveal_info_has->created_at->format('d-m-Y');
                        $reveal_create_date_1month = date('d-m-Y', strtotime($reveal_create_date .  "+1 month"));
                        $reveal_create_date_2month = date('d-m-Y', strtotime($reveal_create_date .  "+2 month"));
                        $reveal_create_date_3month  = date('d-m-Y', strtotime($reveal_create_date .  "+3month"));
                        $last14_days = date('d-m-Y', strtotime($reveal_create_date_3month .  "-14days")); 
                        $reveal_name = $reveal_info_has->name;
                        $updated_at = $reveal_info_has->updated_at;
                        ...
                        // ----------------------------------
                        if(strtotime($today) <= strtotime($reveal_create_date_3month) && strtotime($today) >= strtotime($last14_days))
                        {
                            ...
                            $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Sit tight, you next reveal is coming soon 2 weeks".</h6></div>';
                            $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">NEW REVEAL UPCOMING</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/women_clothing_style_and_fit_section_image.webp') . ');"></div></div>';
                        }

                        elseif ((strtotime($today) < strtotime($last14_days)) && (strtotime($today) >=  strtotime($reveal_create_date_2month))) 
                        {
                            $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name .'</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">New Reveal Upcoming</span>';
                            $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Sit tight, you next reveal is coming soon 2 months".</h6></div>';
                            $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">NEW REVEAL UPCOMING</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/women_clothing_style_and_fit_section_image.webp') . ');"></div></div>';
                        }

                        else if ((strtotime($today) <  strtotime($reveal_create_date_2month)) && (strtotime($today) >=  strtotime($reveal_create_date_1month)))
                        {
                            $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name .'</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">New Reveal Upcoming</span>';
                            $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Sit tight, you next reveal is coming soon 1 month".</h6></div>';
                            $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">NEW REVEAL UPCOMING</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/women_clothing_style_and_fit_section_image.webp') . ');"></div></div>';
                        }
                        // ----------------------------------
                       

                       else if(isset($order_data_info) && (($order_data_info->order_status_id == 1) || ($order_data_info->order_status_id == 3) || ($order_data_info->order_status_id == 4)) &&(($reveal_info_has->status =='preparing_order') || ($reveal_info_has->status == 'dispatched') ) && (($data_obj->status =='preparing_order') || ($data_obj->status =='dispatched')) )
                        {
                        ...
                        else if(($booking_obj->status =='dispatched') && ($reveal_info_has->status =='dispatched') && isset($order_data_info) && ($order_data_info->order_status_id == 6) && (strtotime($today) <= strtotime($update_14_days)))
                        { 
                            ...
                            $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Your items have been. Enjoy your pieces!</h6></div>';
                            $html_booking_info .= '
                        ...
                        else if(isset($order_data_info)  && ($booking_obj->status =='return_initiated') && ($reveal_info_has->status =='return_initiated') && ($order_data_info->order_status_id == 6) && isset($cancel_data) && ($cancel_data->status == 1) && (strtotime($cancel_data->created_at) <= strtotime($update_14_days)))
                        {
                            $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name . '</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">Return Initiated</span>';
                            $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>We\'ve received you\'re return request.</h6></div>';
                            $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;" style="max-width: 300px;">RETURN INITIATED</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/Dappr_smart_casual.jpg') . ');"></div></div>';
                        }
                        else if(($booking_obj->status =='return_initiated') && ($reveal_info_has->status =='return_initiated') && isset($order_data_info) && ($order_data_info->order_status_id == 6) && isset($cancel_data) && ($cancel_data->status == 5 || $cancel_data->status == 6) && (strtotime($cancel_data->created_at) <= strtotime($update_14_days)))
                        {
                            ...
                            $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Sit tight, your next reveal is coming Soon. </h6></div>';
                            $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">REVEAL COMING SOON</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/1_appearance.webp') . ');"></div></div>';
                        }
                    
                        else if(($booking_obj->status == 'dispatched') && ($reveal_info_has->status == 'dispatched') && isset($order_data_info) && ($order_data_info->order_status_id == 6) && !isset($cancel_data) && (strtotime($today)  > strtotime($update_14_days)) )
                        {
                            // ($today > $update_14_days)
                            $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name .'</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">Reveal coming soon</span>';
                            $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Sit tight, you next reveal is coming soon.</h6></div>';
                            $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">REVEAL COMING SOON aa</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/women_clothing_style_and_fit_section_image.webp') . ');"></div></div>';
                        }
                ...
                // }
                // $reveal_info_obj = stylistRevealsItems::where('booking_id', $booking_obj->id)->where('status', $reveal_status)->whereNotIn('id', $order_reveal_ids)->latest('updated_at')->first();
    ...
    public function getCustomerReviewProductsDetails()
    {
        ...
        //      $Cancellation_data = Cancellation::where('order_id', $order_details->id)->where('customer_id', $customer_id)->where('status', 3)->first();
        //      $cacenl_item_arr_inventory_ids = array();
        //      if(isset($Cancellation_data)){
        //         $exchange_more_option = $Cancellation_data->exchange_more_option;
        //         $exchange_more_option_arr = json_decode($exchange_more_option, true);
        //         $cacenl_item_arr_inventory_ids = array_keys($exchange_more_option_arr);
        //      }
        //     if(count($cacenl_item_arr_inventory_ids)){
        //         foreach($cacenl_item_arr_inventory_ids as $cacenl_item_arr_inventory_id){
        //              $Inventory_data = Inventory::where('id', $cacenl_item_arr_inventory_id)->where('shop_id',$Cancellation_data->shop_id)->first();  
        //              if(isset($Inventory_data)){
        //                  if (($key = array_search($Inventory_data->product_id, $order_product_ids)) !== false) {
        ...
        if($order_details){
            ...
            if($inventory_details){
                $item_hide = '';
                $Cancellation_detail =  Cancellation::where('order_id', $order_details->id)->where('status',5)->orWhere('status',6)->first();
            }
        }
        else{
            $buy_product_ids = [];
        ...
        if (is_array($buy_products) && count($buy_products) && isset($buy_products)) {
            foreach ($buy_products as $buy_product_info) {
                ...
                $buy_html .= '<div class="col-md-4  img-product-1 " style="'.$cancel_item_hide.'"><div class="rounded"><img src="' . $buy_product_img_src . '" alt=""  style="width: 100%;"></div><div class="articles-two"><h6>' . $buy_product_obj->name . '</h6><h6>' . $price . '</h6></div></div>';
            }
        }
        else
        {
    ...
    {   
        ...
        $orders = Auth::guard('customer')->user()->orders()->when(request()->q,function(Builder $query){
        ...
        // {
        //     foreach($old_data as $old_data_info)
        //     {
        //         $old_dat = json_decode($old_data_info->items, true);
        //     }
        //     dd($old_dat);
        // }
       
        // dd(json_decode($data_array));
    ...
    {         
        ...
        if($get_booking_data)
        {
            ...
            if($reveal_data)
            {
                stylistRevealsItems::where('booking_id', $get_booking_data->id)->update(['status' => $status_1]);	
            }
        }
    }
}