...
class Order extends BaseModel
{
    ...
    { 
        ...
        $invoice->addSummary(trans('invoice.total'), $this->total - $gst_amount + $this->discount);
        // dd($gst_amount);

        if (($this->taxes || $this->total)) {
            $invoice->addSummary(trans('invoice.taxes') . ' ' . get_formated_decimal(config('app.gst_percentage'), true, 2) . '%' , number_format($this->total - ($this->total * 100)/(100 + config('app.gst_percentage')),2));
        }
        ...
        $invoice->addSummary(trans('invoice.total'), $this->total, true);
