...
class StylistFormController extends Controller
{
                            ...
                            if ($product_obj) {
                                ...
                                if ($decline_options != '') {
                                    ...
                                    $feebback_html .= '<div class="card img-product shadow">
									 ...
									 <div class="rounded">
      										<img src="' . $img_src . '" alt="" style="max-width: 100%;">
   											</div>
                                ...
                                //     {
                                //         $inventy_ids_info =  $inventory_details->id;
                                //     }
                                //     $Cancellation_detail =  Cancellation::where('order_id', $order_details->id)->where('status',5)->latest()->first();
                                //     if(isset($Cancellation_detail->items) && in_array($inventy_ids_info, $Cancellation_detail->items))
                                //     {
                                //         $exchnage_return_option_info = $Cancellation_detail->exchnage_return_option;
                                //         $exchnage_return_option_decde = json_decode($exchnage_return_option_info, true);
                                //         // print_r($exchnage_return_option_decde);
                                //         $force_show = true;
                                //         foreach ($exchnage_return_option_decde as $key => $value)
                                //         {
                                //             if($inventy_ids_info == $key && $value == 'refund' && $force_show == true)
                                //             {
                                //                 $cancel_item_hide = 'display:block';
                                //             }
                                //             else if($inventy_ids_info == $key && $value == 'exchange')
                                //             {
                                ...
                                <div class=" img-product-1 "><div class="rounded "><img src="' . $img_src . '" alt=""  style="max-width: 100%;"></div><div class="articles-two"><h3>' . $brand_name . '</h3><h3>' .$product_obj->name  . '</h3><h3>' . $AttributeValue_size . '</h3><h3>' . $price . '</h3><h3>' . $decline_options_data. '</h3></div><div>' . $other_msg . '</div></div>
                                </div></div>';
    ...
    public function getRevealDetails($reveal_item = null, $reveals_count = 1, $user_details = array())
    {
        ...
        if ($reveal_item) {
            ...
            $revels_html_item_html .= '<img src="' . $add_plus_img . '" alt=""  style="max-width: 100%;">';
    ...
    public function getRevealInfomationHtmlAjax(Request $request)
    {
        ...
        if (is_array($item_and_alernative_html_array)) {
            ...
            if (isset($item_and_alernative_html_array['reveal_status'])) {
                ...
                if (($reveal_status_db == 'awaiting_response') || ($reveal_status_db == 'in_progress') || ($reveal_status_db == 'preparing_order') || ($reveal_status_db == 'dispatched') || ($reveal_status_db == 'return_initiated')) {
    ...
    public function saveRevealItemAddVideo(Request $request)
    {
        ...
        $this->validate($request, [
            'file' => 'required|mimetypes:video/mp4,video/x-matroska,video/avi,video/mpeg,video/quicktime|max:250000',
                ...
                foreach ($details as $detail_info) {
                    ...
                    if (isset($questions_details[$question_id])) {
                        ...
                        if ($q_type == 'budget_calculate_price_update' || $other_text_ans != '') {
                            ...
                            // dd($questions_details[$detail_info->question_id]['name']);
                            // $budget_ques = $questions_details[$detail_info->question_id]['name'];
                            // $replace_text = str_replace("$---","$".$get_budget_price_info , $questions_details[$detail_info->question_id]['name']);
                        }
