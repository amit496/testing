...
class StylistFrontController extends Controller
{
    ...
    public function customerBookingReviewhtml()
    {
        ...
        {   
            ...
            if (!isset($stylist_call_complete) ) {
                ...
                $html .= '<div class="pt-0 mb-4 dappr-text-h"><p>Thanks for booking you call with '. $merchant_name . '</p></div>';
            ...
            elseif(1)
            {
                ...
                {   
                    ...
                    if(isset($reveal_info_obj) && Carbon::parse($reveal_info_obj->updated_at)->diffInDays(Carbon::now()) >= $reveal_show_preview_day && Carbon::parse($reveal_info_obj->updated_at)->diffInDays(Carbon::now()) < $reveal_show_review_if_days_is_more_than && $reveal_info_obj->status != 'completed')
                    {
                        $reveal_id = $reveal_info_obj->id;
                        $reveal_name = $reveal_info_obj->name;
                        $updated_at = $reveal_info_obj->updated_at;
                    ...
                    {       
                        ...
                        $time_count = '';
                        if((strtotime($today) <= strtotime( $booking_obj_3_month)) && (strtotime($today) > strtotime( $booking_obj_2_month_27_days)))
                        {
                            $time_count = "3 Days";
                        }
                        elseif ((strtotime($today) <= strtotime( $booking_obj_2_month_17_days)) && (strtotime($today) > strtotime( $booking_obj_2_month_27_days)))
                        {
                            $time_count = "10 Days"; 
                        }
                        elseif((strtotime($today) <= strtotime( $booking_obj_2_month)) && strtotime($today) > strtotime( $booking_obj_2_month_17_days) )
                        {
                            echo '1 Month';    
                        }
                        else
                        ...
                        if (isset($reveal_info_obj_new) && Carbon::parse($reveal_info_obj_new->updated_at)->diffInDays(Carbon::now()) >= $reveal_show_preview_day && Carbon::parse($reveal_info_obj_new->updated_at)->diffInDays(Carbon::now()) < $reveal_show_review_if_days_is_more_than && $reveal_info_obj_new->status != 'completed')
                        {
                            $reveal_id = $reveal_info_obj_new->id;
                            $reveal_name = $reveal_info_obj_new->name;
                            $updated_at = $reveal_info_obj_new->updated_at;
                            $rand_code = Crypt::encryptString($reveal_id);
                            $booking_id = $booking_obj->id;
                            if ($reveal_name == '')
                            {
                            ...
                            $updated_at = (date("d-m-Y", strtotime($updated_at)));
                            $reveal_name = $reveal_name . ' ' . $updated_at;
                            $html_booking_info = '<div class="pt-0 mb-3 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Your Reveal is ready to view, are you excited as we are?</h6></div>';
                            $reveal_link = url("stylist/reveal/" . $booking_id . "/" . $reveal_id . '/' . $rand_code);
                            $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a href=' . $reveal_link . '>' . 'VIEW YOUR REVEAL' . '</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/') . '/Dappr_animal.jpg?' . rand(10, 1000) . ');"></div></div>';
                            $html_booking_info .= ' <div  class="btn stf_save_btn show_questions_screen_btn col-md-12 p-0"><a href=' . $reveal_link . ' class="btn text-light">' . 'VIEW REVEAL' . '</a></div> </div>';
                        }
                        else if( isset($order_data_info) && (($order_data_info->order_status_id == 1) || ($order_data_info->order_status_id == 3) || ($order_data_info->order_status_id == 4)) &&(($reveal_info_has->status =='preparing_order') || ($reveal_info_has->status == 'dispatched') ) && (($booking_obj->status =='preparing_order') || ($booking_obj->status =='dispatched')) )
                        {
                            $days = stylistRevealResetStatusAfterDayHelper();
                            $date = (Carbon::parse($booking_obj->updated_at)->addDays($days));
                            $days = (Carbon::parse($date)->diffInDays(Carbon::now()));
                            // <h5>Your next Reveal is scheduled for ' . $days . ' days, Stay tuned.</h5>
                            ...
                            if (Carbon::parse($booking_obj->updated_at)->diffInDays(Carbon::now()) > 0) {
                                if (Carbon::parse($booking_obj->updated_at)->diffInDays(Carbon::now()) < stylistRevealResetStatusAfterDayHelper()) {
                                    $days = stylistRevealResetStatusAfterDayHelper() - (int) (Carbon::parse($booking_obj->updated_at)->diffInDays(Carbon::now()));
                                }
                            }
                        }
                        else if(($booking_obj->status =='dispatched') && ($reveal_info_has->status =='dispatched') && isset($order_data_info) && ($order_data_info->order_status_id == 6) &&  ($update_14_days < 14) )
                        { 
                        ...
                        elseif(($booking_obj->status =='return_initiated') && ($reveal_info_has->status =='return_initiated') && isset($order_data_info) && ($order_data_info->order_status_id == 6) && isset($cancel_data) && ($cancel_data->status == 1) && (strtotime($cancel_data->created_at) <= strtotime($update_14_days)))
                        {
                        ...
                        else if(($booking_obj->status =='return_initiated') && ($reveal_info_has->status =='return_initiated') && isset($order_data_info) && ($order_data_info->order_status_id == 6) && isset($cancel_data) && ($cancel_data->status == 3) && (strtotime($cancel_data->created_at) <= strtotime($update_14_days)))
                        {
                        ...
                        else if(($booking_obj->status == 'dispatched') && ($reveal_info_has->status == 'dispatched') && isset($order_data_info) && ($order_data_info->order_status_id == 6) && !isset($cancel_data) && ($update_14_days > 14) )
                        {
                            $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name .'</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">Reveal coming soon</span>';
                        ...
                        else
                        {
                            ...
                            $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Sit tight, you next reveal is coming soon "' .$time_count. '".</h6></div>';
                            $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">NEW REVEAL UPCOMING</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/women_clothing_style_and_fit_section_image.webp') . ');"></div></div>';
                        }
                        // else 
                        // {
                        //     $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name . $two_month_gap. '</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">New Reveal Upcoming </span>';
                        //     $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Sit tight, you next reveal is coming in 2 Months.</h6></div>';
                        //     $html_booking_info .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">REVEAL COMING SOON12345678</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/women_clothing_style_and_fit_section_image.webp') . ');"></div></div>';
                        // }
                    }
                }
            ...
            {
                $html .= '<div class="pt-0 mb-4 dappr-text-h"><h6 >We are current working with your Reveal. Stay tuned.</h6></div>';
                $html .= '<div class=" mb-3 custom_product_img_box pro_img_has  reveal_booked_appointment_info_section"><div class="reveal_booked_appointment_info_section_btn"><a onclick="return false;">REVEAL COMING SOON +++++</a></div><div class="stylist_section_backgound_img" style="background-image: url(' . url('images/stylist/questions/section/booking.jpg') . ');"></div></div>';
            }
    ...
    public function getCustomerReviewProductsDetails()
    {
        ...
        if($order_details){
            ...
            {
                $item_hide = '';
                $Cancellation_detail =  Cancellation::where('order_id', $order_details->id)->where('status',3)->first();
            }
        }
        else
        {
        ...
        if (is_array($buy_products) && count($buy_products) && isset($buy_products)) {
            foreach ($buy_products as $buy_product_info) {
                ...
                $buy_html .= '<div class="col-md-4  img-product-1 " style="'.$cancel_item_hide.'">
										<div class="rounded">
										<img src="' . $buy_product_img_src . '" alt=""  style="width: 100%;">
									</div>
									<div class="articles-two">
								    <h6>' . $buy_product_obj->name . '</h6>
								    <h6>' . $price . '</h6>
								   </div>
								</div>';
            }
