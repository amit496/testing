...
class StylistFrontController extends Controller
{
    ...
    {   
        ...
        // dd($questions_section_heading_arr);
    ...
    public function customerBookingReviewhtml()
    {
        ...
        if($data_obj)
        {
            ...
            $reveal_info_draft_has = stylistRevealsItems::where('booking_id', $data_obj->id)->first();
            // dd($reveal_info_has);
            ...
            elseif(isset($stylist_call_complete) || (isset($reveal_info_draft_has) && $reveal_info_draft_has->status == 'draft'))
            {
                $html ='';
                $html_booking_info ='';
                $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto">
                ...
                if(isset($booking_obj))
                {
                    ...
                    if(isset($reveal_info_has) && (($reveal_info_has->status != 'completed') && ($reveal_info_has->status != 'draft')) )
                    {   
                        $today = date('d-m-Y');
                        ...
                        // var_dump($reveal_info_has->status);
                        // var_dump($booking_obj->status);
                        // var_dump($cancel_data);
                        ...
                        else if(($booking_obj->status =='dispatched') && ($reveal_info_has->status =='dispatched') && isset($order_data_info) && ($order_data_info->order_status_id == 5) && (strtotime($today) <= strtotime($update_14_days)))
                        { 
                           $html = '<div class="container d-flex  reveal_booked_appointment_info"><div class="col-lg-5 col-lg-7 card  mb-5 border-0 m-auto"><div class=""><div class="mb-4 dappr-text-h pt-4"><h3>Welcome ' . $name . '</h3></div><span style="border-bottom:2px solid black; font-weight:600; padding-bottom:5px;">Items Coming</span>';
                            $html_booking_info = '<div class="pt-0 mb-4 dappr-text-h" style="margin-top:1.5rem !important;"><h6>Check your order for status updates.</h6></div>';
                            $html_booking_info .= '
                        ...
                        else if((isset($booking_obj->status) && $booking_obj->status =='return_initiated') && (isset($reveal_info_has->status) && $reveal_info_has->status =='return_initiated') && isset($order_data_info) && ($order_data_info->order_status_id == 6) && isset($cancel_data->status) && (($cancel_data->status == 5) || ($cancel_data->status == 6)) && (strtotime($cancel_data->created_at) <= strtotime($update_14_days)))
                        {
                        ...
                        else if((isset($booking_obj->status) && $booking_obj->status == 'dispatched') && (isset($reveal_info_has->status) && $reveal_info_has->status == 'dispatched') && isset($order_data_info) && ($order_data_info->order_status_id == 6) && !isset($cancel_data) && (strtotime($today)  > strtotime($update_14_days)) )
                        {
    ...
    public function getCustomerReviewProductsDetails()
    {
        ...
        if($order_details){
            ...
            if($inventory_details){
                $item_hide = '';
                $Cancellation_detail =  Cancellation::where('order_id', $order_details->id)->where('status',5)->first();
            }
        }
        else{
            $buy_product_ids = [];
        ...
        if (is_array($buy_products) && count($buy_products) && isset($buy_products)) {
            foreach ($buy_products as $buy_product_info) {
                ...
                // $get_prduct_size
                $buy_html .= '<div class="col-md-4  img-product-1 " style="'.$cancel_item_hide.'"><div class="rounded"><img src="' . $buy_product_img_src . '" alt=""  style="width: 100%;"></div><div class="articles-two"><h6>' . $buy_product_obj->brand . '</h6><h6>' . $buy_product_obj->name . '</h6><h6>' . $get_prduct_size . '</h6><h6>' . $price . '</h6></div></div>';
            }
        }
        else
        {
