...
class Order extends BaseModel
{
    ...
    { 
        ...
        array_unshift($invoiceTo, $this->customer->name . ' '. $this->customer->last_name);
        ...
        $invoice->setColor(config('invoice.color', '#6DBCD4'));      // pdf color scheme
        ...
        $invoice->setReference($this->order_number); 
        // dd($invoice->setReference($this->order_number)) ; // Reference
        ...
        foreach ($this->inventories as $item) {
            $item_desc = explode(',',$item->pivot->item_description);
            // echo "<pre>";
            // print_R($item_desc);
            
            // foreach($item_desc as $key =>  $item_desc_val)
            // {
            //     if((isset($item_desc[0]) && ($item_desc[0] != '') && (array_key_exists($item_desc[0], $item_desc))) || (isset($item_desc[1]) && $item_desc[1] != '') || ((isset($item_desc[4])) && ($item_desc[4] != '') && (array_key_exists($item_desc[4], $item_desc))))
            //     {
            //          echo 'yes';
            //     }
            //     else
            ...
            // var_dump($item_desc[4]);
            // foreach($item_desc as $key =>  $item_desc_info)
            // {
            //     // echo $key  .  ' => '  .$item_desc_info;
            //     if((isset($item_desc[0]) && ($item_desc[0] != '') && (array_key_exists($item_desc[0], $item_desc))) || (isset($item_desc[1]) && $item_desc[1] != ''))
            //     {
            //         $item_description = $item_desc[0];
            //     }
            //     else if((isset($item_desc[0]) && ($item_desc[0] != '') && (array_key_exists($item_desc[0], $item_desc)) || (isset($item_desc[1]) && ($item_desc[1] != '') && (array_key_exists($item_desc[1], $item_desc)))
            //     {
            //         $item_description = $item_desc[0] .','. $item_desc[1];
            //     }
            //     else if((isset($item_desc[0]) && ($item_desc[0] != '') && (array_key_exists($item_desc[0], $item_desc)) || (isset($item_desc[1]) && ($item_desc[1] != '') && (array_key_exists($item_desc[1], $item_desc)) || (isset($item_desc[4]) && ($item_desc[4] != '') && (array_key_exists($item_desc[4], $item_desc)) )
            //     {
            //         $item_description = $item_desc[0] .','. $item_desc[1].','. $item_desc[4];
            //     }
            // }
            foreach ($item_desc as $key => $item_desc_info) {
    if ((isset($item_desc[0]) && $item_desc[0] !== '') )
    {
        $item_description = $item_desc[0];
    } 
    elseif ((isset($item_desc[0]) && $item_desc[0] !== '') || (isset($item_desc[1]) && $item_desc[1] !== ''))
    {
        $item_description = $item_desc[0] . ',' . $item_desc[1];
    } 
    elseif ((isset($item_desc[0]) && $item_desc[0] !== '') || (isset($item_desc[1]) && $item_desc[1] !== '') || (isset($item_desc[4]) && $item_desc[4] !== '')) {
        $item_description = $item_desc[0] . ',' . $item_desc[1] . ',' . $item_desc[4];
    } else {
        ...
        $item_description = 'Default description';
            ...
            $cancel_retunr_approveditems = '';
            $cancel_request_status ='';
            if ($this->cancellation && $this->cancellation->isItemInRequest($item->id)) {
                $cancel_exchange_retunr_option = json_decode($this->cancellation->exchnage_return_option, true);
                $cancel_retunr_approveditems = explode(',',$this->cancellation->approve_items);
                $cancel_request_status = $this->cancellation->status;
                foreach ($cancel_exchange_retunr_option as $key => $values)
                {
                    if(($item->id == $key) && ($values == 'exchange'))
                    {
                        $exchange_return_option = 'Exchange Requested';
                        foreach($cancel_retunr_approveditems as $keys => $refund_approved_items)
                        {
                            if(($item->id == $refund_approved_items) && ($cancel_request_status == 6))
                            {
                                $exchange_return_option = 'Exchange Successful';
                            }
                        }
                    }
                    else if(($item->id == $key) && ($values == 'refund')){
                        $exchange_return_option = 'Refund Requested';
                        foreach($cancel_retunr_approveditems as $keys => $refund_approved_items)
                        {
                            if(($item->id == $refund_approved_items) && ($cancel_request_status == 6))
                            {
                                $exchange_return_option = 'Refund Approved';
                            }
                        }
                    }
                }
                $item_description = substr($item_description, 0, 20) . '... [' . $exchange_return_option . ' ]';
            }

            $invoice->addItem($item_description, '', $item->pivot->quantity, $item->pivot->unit_price);
        }

        $gst_amount = number_format($this->total - ($this->total * 100)/(100 + config('app.gst_percentage')),2);
        $gst_amount = number_format($this->total - ($this->total * 100)/(100 + config('app.gst_percentage')),2);
        $sub_total = $this->total - $gst_amount;
        if($this->discount > 0){
            $gst_amount = number_format((($sub_total-$this->discount)*10)/100,2);
            $contribution_amount =  $sub_total - $this->discount + $gst_amount;
        }else{
            $contribution_amount =  $this->total - $this->discount;
        }

        $sub_total_1 = $this->total - $this->discount;
        ...
        $inven_items_id ='';
        $cancel_exchange_retunr_option = '';
        $cancel_exchnage_items_keys = '';
        $cancel_request_status = '';
        $refund_keys = '';
        $refund_item_amount = 0;
        $refundapproveditems ='';
        foreach ($this->inventories as $item) {
            $inven_items_id = $item->id;
            if(isset($this->cancellation))
            {
                $cancel_exchange_retunr_option = json_decode($this->cancellation->exchnage_return_option, true);
                $cancel_request_status = $this->cancellation->status;
                foreach ($cancel_exchange_retunr_option as $key => $cancel_items_value)
                {
                    if(($item->id == $key) && ($cancel_items_value == 'refund'))
                    {   
                        $refund_keys = $key;
                        $cancel_retunr_approveditems = explode(',',$this->cancellation->approve_items);
                        foreach($cancel_retunr_approveditems as $key =>  $refundapproveditems)
                        {
                            if(($refund_keys == $refundapproveditems) && ($cancel_request_status == 6))
                            {
                                $refund_item_amount = $refund_item_amount + (float)$item->pivot->unit_price;
                            }
                        }
                    }
                }
        ...
        $newval = $this->total - $refund_item_amount;
        // print_r($newval);
        if(($newval != 0)  && ($newval != ''))
        {
            $invoice->addSummary(trans('invoice.total'), $newval);
        }
        else
        {
            $invoice->addSummary(trans('invoice.total'), $this->total);
        }
        // if($refund_item_amount = 0;)
        // $invoice->addSummary(trans('invoice.total'), $this->total);
        ...
        if ($sub_total_2) {
            $invoice->addSummary('Subtotal', $sub_total_2);
        }


        if (($this->taxes || $this->total)) {
            // $invoice->addSummary(trans('invoice.taxes') . ' ' . get_formated_decimal(config('app.gst_percentage'), true, 2) . '%' , $gst_amount);
            // $invoice->addSummary(trans('invoice.taxes ')  , $gst_amount);
            $invoice->addSummary('GST '.config('app.gst_percentage').'%'   , $gst_amount);
        }

        if ($this->shipping == '') {
            $invoice->addSummary(trans('invoice.shipping'), 'Free');
        }
                ...
                foreach ($cancel_exchange_retunr_option as $key => $cancel_items_value)
                {
                    ...
                    {   
                        ...
                        foreach($cancel_retunr_approveditems as $key =>  $refundapproveditems)
                        {
                            if(($refund_keys == $refundapproveditems) && ($cancel_request_status == 6))
                            {
                                $final_refund_item_amount = $final_refund_item_amount + (float)$item->pivot->unit_price;
                            }
                        }
                    }
                }
        ...
        // echo $order_total . 'dsd' . $newval ;
        $newval1 = $order_total - $final_refund_item_amount;
        // print_r($newval);
        if(($newval1 != 0)  && ($newval1 != ''))
        {
            $invoice->addSummary(trans('invoice.total'), $newval1);
        }
       



        // $invoice->addSummary(trans('invoice.total'), $order_total, true);
        // $invoice->addSummary(trans('invoice.total '), $order_total, true);
        // $gst_amount = number_format($this->total - ($this->total * 100)/(100 + config('app.gst_percentage')),2);
    ...
    /**
     * Get the Order request for the Coupon.
     */
    ...
    /**
     * Get the Customer request for the Coupon
     */
