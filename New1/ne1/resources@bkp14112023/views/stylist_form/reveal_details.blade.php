    ...
    @php
        ...
        if(is_array($products_details))
        {
            ...
            foreach($products_details as $product_id_key => $product_details_outer)
            {
                ...
                foreach($product_loop_length as $product_loop_key => $product_details)
                {
                    ...
                    if($attributes->isNotEmpty())
                    {
                        foreach($attributes as $attribute)
                        {
                            ...
                            {
                                $attribute_html .= '<div style="margin-top: 15px;"><b>'.$attribute_name.':</b> '.$attribute_value_text.'</div>';
                            }
                        }
                    }
                    $price =  get_formated_price($price, config('system_settings.decimals', 2));
                    $price_html .= '<div><b>Price:</b> '.$price.'</div>';
                    ...
                    $sty_i++;
                    $product_details_html .= '<div class="col-md-6 pl-md-5  pt-0 p-0   " style="overflow: hidden;"><span class="ref-btn-2" >'. $html_image.' </span>';
                    if($has_alternative_prodcut)
                    {
                        $product_details_html .= '<div class="ref-btn" onclick="stylist_show_alertive_product(this,\''.$product_type_class.'\')"><img src="'.url('images/stylist/sync.png').'" alt="" style="width: 100%;"><span class="pl-2" style="color: #ffffff73;">Alternative</span></div>';
                    }
                    ...
                    $product_details_html .= $alernate_tag.'</div><div class="col-md-6 dappr-altenative-style"><div class="dappr-altenative-style_display_view"><h1 class="mt-4">'.$product_obj->name.'</h1><p> '.$price_html.' </p><p>'.$attribute_html.'</p></div><button type="button" class="btn btn-dark add_cart add_to_card '.$product_is_selected_checked_class.'" name="stylist[\'product_in_cart\'][]"  value="'.$product_obj->id.'" style="display:'.$product_add_cart_btn_display.'" onclick="addtobag_list(this)">ADD TO BAG</button><input type="checkbox" class="style-field-hide  add_to_cart_product_select_'.$product_obj->id.'" name="product_select['.$product_obj->id.']" '.$product_is_selected_checked.'><button type="button" class="btn btn-dark remove_add_to_card " name="stylist[\'product_in_cart_remove\'][]" style="display:'.$product_remove_add_cart_btn_display.'" value="'.$product_obj->id.'" onclick="removetobag_list(this)" >REMOVE FROM BAG</button>';
                    $according_rand_no = $product_obj->id.'_'.rand(1,100);
                    $according_details = '<div class="" id="stylist_accordion_id_'.$according_rand_no.'"><div class=""><div class="dappr-altenative-style_mobiel_view" style="display:none;"><h1 class="mt-4">'.$product_obj->name.'</h1><p>Price '.$price.'</p><p>'.$attribute_html.'</p></div><p class="mt-5">DESCRIPTION</p><div class="stylist-product-description">'.$product_obj->description.'</div></div>';
                    ...
                    if(0 && $attribute_html != '')
                    {
                        $according_details .= '<div class="accordion-item"><h2 class="accordion-header" id="stylist_accordion_item_'.$according_rand_no.'_3"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">More Features</button></h2><div id="collapseThree" class="accordion-collapse collapse" aria-labelledby="stylist_accordion_item_'.$according_rand_no.'_3" data-bs-parent="#stylist_accordion_id_'.$according_rand_no.'"><div class="accordion-body">'.$attribute_html.'</div></div></div>';
                    }
                ...
                $product_details_html .= '<div class=" py-4 px-4 botom-style-previous "><div class="d-flex justify-content-between px-3">'.$previous_btn_text_html.'<a href_rename="javascript:void(0)" onclick="stylist_checkout_from_step_next_show(this);return false;" class="stf_anchor_btn  question_save_btn">'.$next_btn_text.' </a></div>';
                if(count($products_details) == $sty_j)
                {
                    $product_details_html .= ' <a href="javascript:void(0)" class="text-decoration-none px-0"><div class="p-3 my-3 text-center proceed-checkout-style m-auto  w-100"style="   " onclick="stylist_proceed_checkout_page(this)">PROCEED TO CHECKOUT</div></a>';
                }
                $product_details_html .= '</div>';
...
<div class="container-fluid stf_outer_body stylist_step_frontend">
   <form action="{{ url('/stylist/client/submit_selection')}}" id="stylist_proceed_checkout_page" method="post">
        @csrf
        <input type="hidden" name="merchant_id" value="{{ $reveal_info->merchant_id}}">
        <input type="hidden" name="stylist_form_id" value="{{ $reveal_info->id}}">
        <input type="hidden" name="booking_id" value="{{ $reveal_info->booking_id }}">
                ...
                <section class="stylist_step stylist_step_7 " style="display:block;">
                    <div class="container-fluid ">
                        <div class="row my-md-5 align-items-start  ">
                            <div class="col-md-4 sticky-div p-md-0 m-md-0 px-md-5 video_html_outer" >
                                <div class="row">
                                    @php
                                        echo $video_html;
                                    @endphp
                                    <h1 class="py-4 dappr-altenative">Your DAPPR Reveal! <br> Press play for a talk through on what has been chosen and some styling tips</h1>
                            ...
                            <div class="col-md-8 ">
                                <div class="stf_products_list mx-md-4">
                                    @php
                                        echo $product_details_html;
                                    @endphp
                                    <a href="javascript:void(0)" class="text-decoration-none sty_mobile_hide w-100 d-block">
                                        <span class="p-3 my-3 text-center proceed-checkout-style m-auto w-100 d-block " onclick="stylist_proceed_checkout_page(this)">PROCEED TO CHECKOUT</span>
                                    </a>
                                    {{-- <span style="    display: block; text-align: center; padding: 2em 0 3em 0;">Didn't see anything you liked in your Reveal? Contact us here to book a quick chat with your Stylist. <a href="mailto:hello@dappr.com.au">hello@dappr.com.au</a></span> --}}
                                    <span style="display: block; text-align: center; padding: 2em 0 3em 0;">Didn't see anything you liked in your Reveal? Contact us <a href="mailto:hello@dappr.com.au">hello@dappr.com.au</a> here to book a quick chat with your stylist. </span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="container-fluid ">
                        <div class="px-md-3">
                            ...
                            <div class="row pt-md-5 products_feedback_top_section">
                                    ...
                                    <div class="row ">
                                        <div class="">
                                            <div class="col-md-6 p-0">
                                                <h6>FEEDBACK</h6><p>Help us refine your profile by telling us why you chose not to take these items.</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4 px-lg-4 sm-p-my-t offset-md-2 products_feedback_top_section-col-2">
                                    <div class="w-md-75 px-xl-4 px-md-3 py-md-3 border border-dark ">
                                        ...
                                        {{-- <p>Don't want give feedback?<br> Thats fine, hit proceed to checkout to continue</p> --}}
                            ...
                            @php
                                ...
                                foreach($product_feedback_array as $product_id => $product_feedback_info)
                                {
                                    ...
                                    @endphp
                                        <div class="row ">
                                            <div class="col-md-12">
                                                <div class="row product_feedback_item product_feedback_{{$product_id}}" style="display:{{ $product_add_cart_btn_display; }}">
                                                    <div class="col-md-3  p-md-0 m-md-0 px-md-5 " >
                                                        <div class="row">
                                                            {!! $product_feedback_info['html_image'] !!}
                                                        </div>
                                                    </div>
                                                    <div class="col-md-5 ">
                                                        <div class="row">
                                                            <div class="gett-text-ptw stylist_field_outer stylist_field_required_one">
                                                                <h1> {!! $product_feedback_info['name'] !!}</h1>
                                                                <p class=""><b>Price: </b>{!! $product_feedback_info['price'] !!}</p>
                                                                <p class=""><b>Brand: </b>{!! $product_feedback_info['brand'] !!}</p>
                                                                ...
                                                                <span class="row ">
                                                                    ...
                                                                    @endphp
                                                                    @foreach($product_feedback_option_array as $option_key => $option_value)
                                                                        @php
                                                                        ...
                                                                        <div class="col-md-6 style-field-checkbox-outer ">
                                                                            <button type="button" class="btn btn-pr-style-tow style-options my-2 mr-3  field_name_{{$option_key}}  {{ $already_feedback_checked_class }}">{{$option_value}}</button>
                                                                            <input type="checkbox" class="style-field-hide style-options-checkbox stylist_field_required '". $product_is_selected_checked_class ."'" name="stylist_feedback[{{$product_id}}][{{$option_key}}]" value="{{$option_key}}" {{ $already_feedback_checked }}>
                                                                        </div>
                                                                    @endforeach
                                                                    <div class="col-sm-12">
                                                                        <textarea  class="style-field-hide form-control other_feedback_field" name="stylist_feedback_other[{{$product_id}}][other]" {!! $other_text_msg_style !!}>{{$other_text_msg}}</textarea>
                                                                    </div>
                                                                </span>
                                                            </div>
                                                        </div>
                            ...
                            <a href="javascript:void(0)" class="text-decoration-none">
                                <div class="p-3 my-3 text-center proceed-checkout-style m-auto "style="width: 100%;" onclick="stylist_proceed_feedback_page(8,8)">PROCEED TO CHECKOUT</div>
                            </a>
                        </div>
                    </div>
                </section>
