   ...
   if(is_array($products_details))
   {
      ...
      foreach($products_details as $product_id_key => $product_details_outer)
      {
         ...
         foreach($product_loop_length as $product_loop_key => $product_details)
         {
            ...
            $product_details_html .= $alernate_tag.'</div><div class="col-md-6 dappr-altenative-style"><div class="dappr-altenative-style_display_view"><h1 class="mt-4">'.$product_obj->name.'</h1><p>'.$price_html.'</p><p>'.$attribute_html.'</p></div>
         ...
         $product_details_html .= '<div class=" py-4 px-4 botom-style-previous ">
         <div class="d-flex justify-content-between px-3">'.$previous_btn_text_html.'<a href_rename="javascript:void(0)" onclick="stylist_checkout_from_step_next_show(this);return false;" class="stf_anchor_btn  question_save_btn">'.$next_btn_text.' </a></div>';
         if(count($products_details) == $sty_j)
         {
