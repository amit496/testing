                                    ...
                                    if (is_array($buy_products) && count($buy_products))
                                    {
                                        foreach ($buy_products as $buy_product_info)
                                        {
                                            ...
                                            if (isset($item_cancel_item_hide->items) && in_array($buy_inventory_obj->id,  $item_cancel_item_hide->items))
                                            {
                                                ...
                                                //     }
                                                // }
                                                if(array_key_exists($buy_inventory_obj->id, $exchnage_return_option_decde) && $exchnage_return_option_decde[$buy_inventory_obj->id] == 'exchange')
                                                {
                                                    $cancel_item_hide = 'display:none;';
                                                }
                                    ...
                                    if (is_array($buy_products) && count($buy_products))
                                    {
                                        foreach ($buy_products as $buy_product_info)
                                        {
                                            ...
                                            if (isset($item_cancel_item_hide->items) && in_array($buy_inventory_obj->id, $item_cancel_item_hide->items))
                                            {
                                                ...
                                                else if(array_key_exists($buy_inventory_obj->id, $exchnage_return_option_decde) && ($exchnage_return_option_decde[$buy_inventory_obj->id] == 'refund') && in_array($buy_inventory_obj->id,$decline_refund ))
                                                {
                                                    $cancel_item_hide = 'display:none;';
                                                }
                                                else if(array_key_exists($buy_inventory_obj->id, $exchnage_return_option_decde) && ($exchnage_return_option_decde[$buy_inventory_obj->id] == 'refund') && in_array($buy_inventory_obj->id,$approved_refund ))
                                            ...
                                            $buy_html .= '<div class="col-md-2 img-product-1" style ="' . $cancel_item_hide . '"><div class="rounded "><img src="' . $buy_product_img_src . '" alt="" style="max-width: 100%;"></div><div class="articles-two"><h3>' . $buy_product_obj->name . '</h3><h3>' . $product_brand_name . '</h3><h3>' . $price . '</h3><h3>' . $description . '</h3><h3>' . $return_more_suboption . '</h3></div><div></div></div>';
                                        }
