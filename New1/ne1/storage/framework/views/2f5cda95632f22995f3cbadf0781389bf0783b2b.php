    ...
    $(document).ready(function() {
      ...
      if ($("#checkout-id").length == 1) {
        ...
        if ($('.payment-option').length == 0) {
          disableCartPayment('<?php echo e(trans('theme.notify.seller_has_no_payment_method'), false); ?>');
          $('#payment-instructions').children('span').html('<?php echo e(trans('theme.notify.seller_has_no_payment_method'), false); ?>');
        }
        ...
        $("select#address_country").on('change', function() {
          // Show shipping charge may change msg if zone changes
          $(this).next('.help-block').html('<small><?php echo e(trans('theme.notify.shipping_cost_may_change'), false); ?></small>');
        });

        $("select#address_state").on('change', function(e) {
          if (!$("select#address_country").next('.help-block').text()) {
            $(this).next('.help-block').html('<small><?php echo e(trans('theme.notify.shipping_cost_may_change'), false); ?></small>');
          }
        ...
        $.ajax({
          ...
          success: function(result) {
            ...
            if (result.length === 0) {
              ...
              var options = '<option value=""><?php echo e(trans('theme.select'), false); ?></option>';
              for (var n in result) {
      ...
      function setShippingZone(cart, country_id, state_id) {
        ...
        if ($.isEmptyObject(zone)) {
          <?php echo $__env->make('theme::layouts.notification', ['message' => trans('theme.notify.seller_doesnt_ship'), 'type' => 'warning', 'icon' => 'times-circle'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
        }
      ...
      $('.cart-item-remove').on('click', function(e) {
        e.preventDefault();
        var node = $(this);
        console.log(node + "dsjkhfjsdhfjksdhf");
        var cart = $(this).data('cart');
        console.log(cart  +  'dnbdf');
        var item = $(this).data('item');
        console.log(item + 'sdfsjkd');

        $.ajax({
          url: '<?php echo e(route('cart.removeItem'), false); ?>',
          type: 'POST',
          data: {
            'cart': cart,
            'item': item
          ...
          complete: function(xhr, textStatus) {
            if (200 == xhr.status) {
              <?php echo $__env->make('theme::layouts.notification', ['message' => trans('theme.notify.item_removed_from_cart'), 'type' => 'success', 'icon' => 'check-circle'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
      ...
      $('.apply_seller_coupon').on('click', function(e) {
        ...
        if (coupon) {
          $.ajax({
              url: '<?php echo e(route('validate.coupon'), false); ?>',
              type: 'POST',
              data: {
                'coupon': coupon,
                'shop': shop,
            ...
            .fail(function(response) {
              if (401 === response.status) {
                ...
                <?php echo $__env->make('theme::layouts.notification', ['message' => trans('theme.notify.coupon_not_valid'), 'type' => 'warning', 'icon' => 'times-circle'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
              } else if (404 === response.status) {
                <?php echo $__env->make('theme::layouts.notification', ['message' => trans('theme.notify.coupon_not_exist'), 'type' => 'danger', 'icon' => 'times-circle'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
              } else if (443 === response.status) {
                <?php echo $__env->make('theme::layouts.notification', ['message' => trans('theme.notify.coupon_not_valid_for_zone'), 'type' => 'warning', 'icon' => 'times-circle'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
              } else if (444 === response.status) {
                <?php echo $__env->make('theme::layouts.notification', ['message' => trans('theme.notify.coupon_limit_expired'), 'type' => 'warning', 'icon' => 'times-circle'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
              } else {
                <?php echo $__env->make('theme::layouts.notification', ['message' => trans('theme.notify.failed'), 'type' => 'danger', 'icon' => 'times-circle'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
              }
      ...
      var apply_btn = '<div class="space5"></div><button class="popover-submit-btn btn btn-black btn-block flat" type="button"><?php echo e(trans('theme.button.ok'), false); ?></button>';
      ...
      $(document).on("click", ".popover-submit-btn", function() {
        ...
        switch (nodeId) {
          case 'shipping-options-popover':
            ...
            var name = shipping.attr('id') == 'custom_shipping' ? '<?php echo e(trans('theme.custom_shipping'), false); ?>' : shipping.attr('id');
      ...
      $('a.packaging-options').popover({
        ...
        content: function() {
          ...
          var preChecked = String(current) == String('<?php echo e(trans('theme.basic_packaging'), false); ?>') ? 'checked' : '';

          var options = '<table class="table table-striped">' +
            '<tr><td><div class="radio"><label id="1"><input type="radio" name="packaging_option" id="<?php echo e(trans('theme.basic_packaging'), false); ?>" value="' + getFormatedValue(0) + '" ' + preChecked + '>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo e(trans('theme.basic_packaging'), false); ?></label></div></td>' +
            '<td><span><?php echo e(get_currency_prefix(), false); ?>' + getFormatedValue(0) + '<?php echo e(get_currency_suffix(), false); ?></span></td></tr>';
          ...
          $(this).data('options').forEach(function(item) {
            ...
            options += '<tr><td><div class="radio"><label id="' + item.id + '"><input type="radio" name="packaging_option" id="' + item.name + '" value="' + getFormatedValue(item.cost) + '" ' + preChecked + '>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + item.name + '</label></div></td>' +
              '<td><span><?php echo e(get_currency_prefix(), false); ?>' + getFormatedValue(item.cost) + '<?php echo e(get_currency_suffix(), false); ?></span></td></tr>';
          });
          options += '</table>';
      ...
      $('a.dynamic-shipping-rates').popover({
        ...
        content: function() {
          ...
          var preChecked = String(current) == String('<?php echo e(trans('theme.free_shipping'), false); ?>') ? 'checked' : '';
          ...
          if ($.isEmptyObject(filtered)) {
            var options = '<p class="space10"><span class="space10"></span><?php echo e(trans('theme.seller_doesnt_ship'), false); ?></p>';
          } else {
            var options = '<table class="table table-striped" id="checkout-options-table">';

            if (free_shipping) {
              options += '<tr><td><div class="radio"><label id="0"><input type="radio" name="shipping_option" id="<?php echo e(trans('theme.free_shipping'), false); ?>" value="' + getFormatedValue(0) + '" ' + preChecked + '>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo e(trans('theme.free_shipping'), false); ?></label></div></td>' +
                '<td>&nbsp;</td><td>&nbsp;</td>' +
                '<td><span><?php echo e(get_currency_prefix(), false); ?>' + getFormatedValue(0) + '<?php echo e(get_currency_suffix(), false); ?></span></td></tr>';
            }
            ...
            filtered.forEach(function(item) {
              ...
              options += '<tr><td><div class="radio"><label id="' + item.id + '"><input type="radio" name="shipping_option" id="' + item.name + '" value="' + getFormatedValue(item.rate) + '" ' + preChecked + '>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + item.name + '</label></div></td>' +
                ...
                '<td><span><?php echo e(get_currency_prefix(), false); ?>' + getFormatedValue(shippingRate) + '<?php echo e(get_currency_suffix(), false); ?></span></td></tr>';
            });
            options += '</table>';
          }
      ...
      function calculateDiscount(cart) {
        ...
        if (coupon.min_order_amount && totalPrice < coupon.min_order_amount) {
          <?php echo $__env->make('theme::layouts.notification', ['message' => trans('theme.notify.coupon_min_order_value'), 'type' => 'danger', 'icon' => 'times-circle'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
        ...
        if ('percent' == coupon.type) {
          ...
          name += ' (<?php echo e(get_currency_prefix(), false); ?>' + getFormatedValue(coupon.value) + '<?php echo e(get_currency_suffix(), false); ?>)';
        }
      ...
      function setShippingOptions(cart) {
        ...
        if (!$.isEmptyObject(filtered)) {
          ...
          if (isFreeShipping(cart)) {
            setShippingCostThenSave(cart, '<?php echo e(trans('theme.free_shipping'), false); ?>', 0, 0);
          } else {
      ...
      function applyDiscountOnCart(cart, coupon = null) {
        if (coupon == null) {
          <?php echo $__env->make('theme::layouts.notification', ['message' => trans('theme.notify.coupon_not_valid'), 'type' => 'danger', 'icon' => 'times-circle'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
        ...
        <?php echo $__env->make('theme::layouts.notification', ['message' => trans('theme.notify.coupon_applied'), 'type' => 'success', 'icon' => 'check-circle'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
      ...
      function disableCartCheckout(cart) {
        disableCartPayment("<?php echo e(trans('theme.notify.seller_doesnt_ship'), false); ?>"); // For checkout page only
      ...
      function updateCartOnServerside(cart) {
        let temproute = "<?php echo e(route('cart.update', '_CART_'), false); ?>";
        ...
        $.ajax({
          ...
          .fail(function(response) {
            ...
            <?php echo $__env->make('theme::layouts.notification', ['message' => trans('theme.cart_update_failed'), 'type' => 'warning', 'icon' => 'times-circle'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
          });
...
<?php /**PATH /home/dappr/public_html/portal.dappr.com.au/public/themes/default/views/scripts/dynamic_checkout.blade.php ENDPATH**/ ?>