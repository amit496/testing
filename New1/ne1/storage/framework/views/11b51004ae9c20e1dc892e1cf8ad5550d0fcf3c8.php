  ...
  <div id="chat-window" class="chat">
    <div class="chat_header">
      <div class="chat_option">
        <div class="header_img">
          <img src="<?php echo e(get_storage_file_url(optional($shop->image)->path, 'thumbnail'), false); ?>" />
        </div>
        <span id="chat_head"><?php echo e($shop->name, false); ?></span> <br>
        <span class="agent"><?php echo e($agent->getName(), false); ?></span> <span class="online">(<?php echo e($agent_status, false); ?>)</span>
      </div>
    </div>
    ...
    <div id="chat_conversation" class="chat_converse">
      ...
      <?php if (! (Auth::guard('customer')->check())): ?>
        <p>
          <?php echo trans('theme.login_to_chat'); ?>

          <a href="javascript:void(0)" class="btn btn-primary" data-toggle="modal" data-target="#loginModal"><?php echo e(trans('theme.button.login'), false); ?></a>
        </p>
      <?php else: ?>
        <p class="text-primary"><?php echo trans('theme.connecting'); ?></p>
      <?php endif; ?>
    </div>
...
<script type="text/javascript">
  ...
  $('<img/>').attr('src', "<?php echo e(get_storage_file_url(optional($shop->image)->path, 'thumbnail'), false); ?>").appendTo(agent_avatar);
  ...
  (function($, window, document) {
    $(document).ready(function() {
      ...
      function sendTheMessage() {
        ...
        var response = '';
        $.ajax({
          url: "<?php echo e(route('chat.start'), false); ?>",
          type: 'POST',
          data: {
            'message': msg,
            'shop_slug': "<?php echo e($shop->slug, false); ?>",
          },
          complete: function(xhr, textStatus) {
            ...
            switch (xhr.status) {
              ...
              case 401:
                ...
                response = $('<p>').addClass('text-danger').text("<?php echo trans('theme.login_to_chat'); ?>");
                $('<br/><br/>').prependTo(response);
                $('<a>').attr('href', "javascript:void(0)").attr('data-toggle', "modal").attr('data-target', "#loginModal").addClass('btn btn-primary').text("<?php echo e(trans('theme.button.login'), false); ?>").appendTo(response);
                break;

              case 403:
              case 419:
                $("#chat_conversation").html(""); //Clear the chatbox
                response = $('<p>').addClass('text-danger').text("<?php echo trans('theme.session_expired'); ?>");
                $('<br/><br/>').prependTo(response);
                $('<a>').attr('href', "javascript:void(0)").attr('data-toggle', "modal").attr('data-target', "#loginModal").addClass('btn btn-primary').text("<?php echo e(trans('theme.button.login'), false); ?>").appendTo(response);
                break;

              case 404:
                response = $('<p>').addClass('text-danger').text("<?php echo trans('theme.shop_not_found'); ?>");
                $('<br/><br/>').prependTo(response);
                $('<a>').attr('href', "/").addClass('btn btn-primary').text("<?php echo e(trans('theme.button.shop_now'), false); ?>").appendTo(response);
                break;

              default:
                response = $('<p>').addClass('text-danger').text("<?php echo trans('theme.notify.failed'); ?>");
                $('<br/><br/>').prependTo(response);
            }
      ...
      function loadOldChat() {
        $.ajax({
          url: "<?php echo e(route('chat.conversation', $shop->id), false); ?>",
          success: function(result) {
            ...
            if (result) {
              ...
              response.addClass('chat_msg_item_admin').text("<?php echo trans('theme.chat_welcome'); ?>");
...
<?php if(Auth::guard('customer')->check()): ?>
  ...
  <script type="text/javascript">
    ...
    (function($, window, document) {
      $(document).ready(function() {
        var pusher = new Pusher('<?php echo e(config('services.pusher.key'), false); ?>', {
          cluster: '<?php echo e(config('services.pusher.cluster'), false); ?>',
          forceTLS: true
        });

        var channel = pusher.subscribe('<?php echo e(get_chat_room_name($shop->id . Auth::guard('customer')->user()->id), false); ?>');
...
<?php /**PATH /home/dappr/public_html/portal.dappr.com.au/public/themes/default/views/scripts/chatbox.blade.php ENDPATH**/ ?>