...
<script type="text/javascript">
  ...
  (function($, window, document) {
    let free_shipping = '<?php echo e($item->free_shipping, false); ?>';
    let shop_id = '<?php echo e($item->shop_id, false); ?>';
    let handlingCost = getFromPHPHelper('getShopConfig', [shop_id, 'order_handling_cost']);
    let unitPrice = '<?php echo e($item->current_sale_price(), false); ?>';
          ...
          if ($.isEmptyObject(filtered)) {
            ...
            if (free_shipping) {
              options += "<tr><td><div class='radio'><label id='0' data-option='" + JSON.stringify({
                  name: '<?php echo e(trans('theme.free_shipping'), false); ?>',
                  rate: 0
                }) + "'><input type='radio' name='shipping_option' id='<?php echo e(trans('theme.free_shipping'), false); ?>' value='" + getFormatedValue(0) + "' " + preChecked + "/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo e(trans('theme.free_shipping'), false); ?></label></div></td>" +
                '<td>&nbsp;</td>' +
                '<td><small class"text-muted"><?php echo e(trans('theme.std_delivery_time'), false); ?></small></td>' +
                '<td><span><?php echo e(trans('app.free'), false); ?></span></td></tr>';
            }
    ...
    function setItemDetails(item) {
      ...
      $('#item_shipping_weight').html(details.shipping_weight + ' ' + "<?php echo e(config('system_settings.weight_unit'), false); ?>");
    }
      ...
      $.ajax({
        ...
        success: function(result) {
          ...
          if (result.length === 0) {
            ...
            let options = '<option value=""><?php echo e(trans('theme.select'), false); ?></option>';
            for (let n in result) {
    ...
    function setSalePrice(item) {
      ...
      if (
        ...
        itemWrapper.find('.percent-off').show().html(getFormatedValue(off, 0) + '<?php echo e(trans('theme.percnt_off'), false); ?>');
      } else {
    ...
    function setStockQuantity(item) {
      ...
      itemWrapper.find('.product-info-availability span').text('<?php echo e(trans('theme.in_stock'), false); ?>');
      itemWrapper.find('.product-info-title').html(item.title);
      itemWrapper.find('.available-qty-count').text(item.stock_quantity + ' <?php echo e(strtolower(trans('theme.in_stock')), false); ?>');
    ...
    function setOutOfStock() {
      itemWrapper.find('.product-info-availability span').html('<b class="text-danger"><?php echo e(trans('theme.out_of_stock'), false); ?></b>');
      itemWrapper.find('.product-info-price-new').text('<?php echo e(trans('theme.out_of_stock'), false); ?>');
    ...
    function getShippingWeight() {
      let unit_weight = '<?php echo e($item->shipping_weight, false); ?>';
      let qtt = $('input.product-info-qty-input').val();
    ...
    function setShippingCost(shipping) {
      ...
      if (free_shipping == 1 && shipping.rate == 0) {
        ...
        $('#delivery-time').text('<?php echo e(trans('theme.std_delivery_time'), false); ?>');
        ...
        if (shipping.carrier.name != ' ') {
          $('#summary-shipping-carrier').text(' <?php echo e(strtolower(trans('theme.by')), false); ?> ' + shipping.carrier.name);
        } else {
          $('#summary-shipping-carrier').text(' ');
        }

        let delivery_takes = shipping.delivery_takes ? '<?php echo e(trans('theme.estimated_delivery_time'), false); ?>: ' + shipping.delivery_takes : '';
    ...
    function setShippingOptions() {
      $.when(
        $('#summary-shipping-cost, #summary-total').removeClass('lead text-uppercase').html('<?php echo e(trans('theme.notify.calculating'), false); ?>')
      ).then(function() {
    ...
    function canNotDeliver() {
      $('#summary-shipping-cost, #summary-total').removeClass('lead').addClass('text-danger text-uppercase').html('<?php echo e(trans('theme.notify.cant_deliver'), false); ?>');
      $('#summary-shipping-carrier').text(' ');
...
<?php /**PATH /home/dappr/public_html/portal.dappr.com.au/public/themes/default/views/scripts/product_page.blade.php ENDPATH**/ ?>