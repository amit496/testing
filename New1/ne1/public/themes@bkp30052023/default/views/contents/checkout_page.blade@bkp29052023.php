    ...
    <div class="row shopping-cart-table-wrap space30" id="cartId{{ $cart->id }}" data-cart="{{ $cart->id }}">
      <div class="col-md-4 bg-light">
        ...
        <div class="input-group full-width space30">
          ...
          <input name="coupon" value="" id="coupon{{ $cart->id }}" class="form-control flat" type="text" placeholder="@lang('theme.placeholder.have_coupon_from_seller')">

          <span class="input-group-btn">
            <button class="btn btn-default flat apply_seller_coupon" type="button" data-cart="{{ $cart->id }}" style="padding-left:40px;padding-right:40px;font-weight: 300;">@lang('theme.button.apply')</button>
          </span>
        </div><!-- /input-group -->
        ...
        @endphp
          ...
          <li>
            ...
            <span>{{ get_currency_prefix() }}
              <span id="summary-total{{ $cart->id }}" class="item-total{{ $cart->id }}">{{ number_format($sub_total, 2, '.', '') }}</span>{{ get_currency_suffix() }}
            </span>
          </li>
          <li>
            <span>
          ...
          <li id="discount-section-li{{ $cart->id }}" style="display: <?php if($cart->discount > 0 ){ echo 'block !important' ; } else { echo 'none' ;}?>;">
            <span>{{ trans('theme.company_contribution') }}
              ...
              <i><em id="summary-discount-name{{ $cart->id }}" class="small text-muted">${{$cart->coupon ? number_format($cart->discount, 0, '.', '') : number_format(0, 2, '.', '') }} off by company (${{$cart->coupon ? number_format($cart->discount, 0, '.', '') : number_format(0, 2, '.', '')}})</em></i>
            </span>
            {{-- ${{ number_format($order->grand_total,  2 , '.', '') }}$cart->discount --}}
            <span>{{get_currency_prefix()}}
            <span id="summary-discount{{ $cart->id }}">{{ $cart->coupon ? number_format($cart->discount, 0, '.', '') : number_format(0, 2, '.', '') }}</span>
            </span>
          </li>
          ...
          <li>
            ...
            <span class="lead">{{ get_currency_prefix() }}
              <span id="summary-grand-total{{ $cart->id }}12">{{ number_format($contribution_amount, 2, '.', '') }}</span>{{ get_currency_suffix() }}
            </span>
          </li>
        </ul>
