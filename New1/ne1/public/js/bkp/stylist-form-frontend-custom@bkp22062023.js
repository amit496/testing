...
//    console.log("current_step_no 276: "+current_step_no);
		...
		if(field_validation){
			...
			if(not_skip_validaiton_and_saving_part){
	            ...
	            if(!save_response.success){
	            	// stfErrorAlert('something wrong','erro');
	            	return false;
	            }
	...
	if(image_selector.length == 1){
		...
		console.log("stfUploadfileShow 1111111");
			...
			if (image && image[0]) {
					...
					if(!(/\.(png|jpg|jpeg,heic,)$/i).test(image_selector.val())) {
...
function stylist_show_questions_answers_screen(){
	...
	//jQuery('.stf_questeions_screen_wrappr .stylist_qeustions_list .stylist_step_progress_active_already:last').trigger('click');
