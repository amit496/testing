    ...
    */
    ...
    public function getRevealItemStep3Html($booking_id = 0, $reveal_id = 0)
    {
        ...
        if (isset($booking_details)) {
            ...
            $reveals_info = stylistRevealsItems::where('merchant_id', $booking_details->merchant_id)->where('booking_id', $booking_details->id)->orderby('id', 'desc')->skip(1)->take(1)->first();
            // var_dump($reveals_info);
            if(isset($reveals_info)  &&  ($reveals_info !=  ''))
            {

                $futureDate = date('d-m-Y', strtotime($reveals_info->created_at .  "90 days"));
                $today_date = date('d-m-y');
                // echo strtotime($today_date) . "<br>";
                if(strtotime($today_date) >= strtotime($futureDate) )
                {
                    $send_button_hide = 'display:block';
                }
                else
                {
                    $send_button_hide = 'display:none';
                }
                // dd(strtotime($futureDate));


                // $formattedDate = $futureDate->format('d-m-y');
                // if(time() >= strtotime($formattedDate))
                // {
                //     echo 'yes<br>';
                //     echo time(). "<br>";
                //     echo strtotime(date('d-m-y')). ' - '. date('d-m-y'). "<br>";
                //     echo strtotime($formattedDate). ' - ' . $formattedDate."<br>";
                // }
        ...
        if ($show_send_notification_btn == 'Y') {
            $step3_html .= '<div class="position-absolute-styles top-0 end-0 btn_send_user_notification">
              	<a data-link="' . url('') . '/admin/stylist/customer_request_response/load_email_template/' . $booking_id . '/' . $reveal_id . '" class="ajax-modal-btn " style="cursor: pointer; '.$send_button_hide.';">SEND TO  ' . $client_name . '</a>
            </div>';
        }
