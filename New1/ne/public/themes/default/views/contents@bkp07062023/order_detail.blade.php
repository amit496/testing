        ...
        <table class="table" id="buyer-payment-detail-table">
          ...
          <tbody>
            <tr class="buyer-payment-info-head" style="background: #6DBCD4;">
              {{-- <td>@lang('theme.amount')</td> --}}
              <td>@lang('theme.total_amount')</td>
              <td>@lang('theme.company_contribution')</td>
              <td>@lang('theme.subtotal')</td>
              <td colspan="2">@lang('theme.gst')</td>
              <td>@lang('theme.shipping')</td>
              {{-- <td>@lang('theme.packaging_cost')</td> --}}
              
              
              {{-- <td colspan="2">@lang('theme.total')</td> --}}
            </tr>
              @php
            ...
            <tr class="buyer-payment-info-body">
              ...
              <td>${{ number_format($sub_total_2,  2 , '.', '') }}</td>
              <td colspan="2">${{ number_format($gst_amount,  2 , '.', '') }}</td>
              <td>Free</td>
              {{-- <td>${{ number_format($order->shipping + $order->handling,  2 , '.', '') }}</td> --}}
              {{-- <td>${{ number_format($order->packaging,  2 , '.', '') }}</td> --}}
            ...
            <tr class="buyer-payment-info-head" style=" background: #6DBCD4;">
              <td colspan="2">@lang('theme.order_amount')</td>
              <td colspan="">@lang('theme.payment_method')</td>
              <td colspan="3">@lang('theme.status')</td>
            </tr>

            <tr class="buyer-payment-info-body">
              {{-- <td colspan="2">{{ get_formated_currency($order->grand_total, true, 2) }}</td> --}}
              <td colspan="2">${{ number_format($order_total,  2 , '.', '') }}</td>
              <td colspan="">{{ $order->paymentMethod->name }}</td>
              <td colspan="3">{!! $order->paymentStatusName() !!}</td>
            </tr>
          </tbody>
        </table>
      </div><!-- /.col-md-12 -->
          ...
          <tbody>
            ...
            <tr class="order-info-head">
              <td width="40%">
                ...
                <h5 class="mt-2">
                  <span style="color:#6DBCD4;">@lang('theme.order_time_date'): </span>{{ $order->created_at->toDayDateTimeString() }}
                </h5>
              </td>
              ...
              <td width="20%" class="order-amount">
                <h5 class="my-1">
                  ...
                  <span  style="color:#6DBCD4;">@lang('theme.order_amount'): </span>{{ '$'.number_format($order_total, 2 ,'.', '') }}
                </h5>
              </td>
            </tr> <!-- /.order-info-head -->
            ...
            <?php 
              ...
              foreach ($convert_item_data as $key => $convert_item_data_value) {
                if(isset($convert_item_data[0]))
                {
                  $title = $convert_item_data[0];
                }
                if(isset($convert_item_data[3]))
                {
                  $size = $convert_item_data[3];
                }
              }
            ?>
              <tr class="order-body">
                ...
                @if ($loop->first)
                  <td rowspan="{{ $loop->count }}" class="order-actions">
                    <a href="{{ url('stylist/order/cancel') }}" style="display:block; width:100%; padding:5px 10px; font-size:12px; line-height:1.5;    color: #333; text-align: center;background:#fff; border:1px solid #ccc; margin-bottom:5px">Exchange/Return</a>
                    ...
                    {{-- <a href="{{ route('dispute.open', $order) }}" class="confirm btn btn-sm btn-block btn-default flat"
                      data-confirm="@lang('theme.confirm_action.open_a_dispute')">@lang('theme.button.Exchange_Return')</a> --}}
                    ...
                    @unless($order->isCanceled())
                      ...
                      @if ($order->canBeCanceled())
                        ...
                        {!! Form::button('<i class="fas fa-times-circle-o"></i> ' . trans('theme.cancel_order'), ['type' => 'submit', 'class' => 'confirm btn btn-default d-none btn-block flat', 'data-confirm' => trans('theme.confirm_action.cant_undo')]) !!}
                        {!! Form::close() !!}

                      @elseif($order->canRequestCancellation())

                        <a href="{{ route('cancellation.form', ['order' => $order, 'action' => 'cancel']) }}" class="modalAction btn btn-default btn-sm d-none btn-block flat"><i class="fas fa-times"></i> @lang('theme.cancel_items')</a>

                      @endif

                      @if ($order->canTrack())
                        <a href="{{ route('order.track', $order) }}" class="btn btn-black btn-sm btn-block flat d-none">
                          <i class="fas fa-map-marker"></i> @lang('theme.button.track_order')
                        </a>
                      @endif

                      @if ($order->canEvaluate())
                        <a href="{{ route('order.feedback', $order) }}" class="btn btn-primary btn-sm btn-block flat d-none">
                      ...
                      @if ($order->isFulfilled())
                        @if ($order->canRequestReturn())
                          <a href="{{ route('cancellation.form', ['order' => $order, 'action' => 'return']) }}" class="modalAction btn btn-default btn-sm btn-block flat d-none"><i class="fas fa-undo"></i> @lang('theme.return_items')</a>
                        @endif

                        @unless($order->goods_received)
                          {!! Form::model($order, ['method' => 'PUT', 'route' => ['goods.received', $order]]) !!}
                          {!! Form::button(trans('theme.button.confirm_goods_received'), ['type' => 'submit', 'class' => 'confirm btn btn-primary btn-block flat d-none', 'data-confirm' => trans('theme.confirm_action.goods_received')]) !!}
                          {!! Form::close() !!}
                        @endunless
                      @endif
