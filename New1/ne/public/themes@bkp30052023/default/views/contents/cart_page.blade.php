...
<section>
    ...
    <div class="container">
        ...
        <div class="row shopping-cart-table-wrap mb-5 mt-3 {{ $expressId == $cart->id ? 'selected' : '' }}"
            id="cartId{{ $cart->id }}" data-cart="{{ $cart->id }}">
            <div class="col-md-9">
                {!! Form::model($cart, ['method' => 'PUT', 'route' => ['cart.checkout', $cart->id], 'id' => 'formId' .
                $cart->id]) !!}
                {{ Form::hidden('cart_id', $cart->id, ['id' => 'cart-id' . $cart->id]) }}
                {{ Form::hidden('shop_id', $cart->shop->id, ['id' => 'shop-id' . $cart->id]) }}
                {{ Form::hidden('tax_id', isset($shipping_zones[$cart->id]->id) ? $shipping_zones[$cart->id]->tax_id :
                null, ['id' => 'tax-id' . $cart->id]) }}
                {{ Form::hidden('taxrate', null, ['id' => 'cart-taxrate' . $cart->id]) }}
                {{ Form::hidden('packaging_id', $default_packaging ? $default_packaging->id : null, ['id' =>
                'packaging-id' . $cart->id]) }}
                {{ Form::hidden('ship_to', $cart->ship_to, ['id' => 'ship-to' . $cart->id]) }}
                {{ Form::hidden('shipping_zone_id',isset($shipping_zones[$cart->id]->id) ?
                $shipping_zones[$cart->id]->id : null,['id' => 'zone-id' . $cart->id]) }}
                {{ Form::hidden('shipping_rate_id', $cart->shipping_rate_id, ['id' => 'shipping-rate-id' . $cart->id])
                }}
                {{ Form::hidden('ship_to_country_id', $cart->ship_to_country_id, ['id' => 'shipto-country-id' .
                $cart->id]) }}
                ...
                {{ Form::hidden('handling_cost', optional($cart->shop->config)->order_handling_cost, ['id' =>
                'handling-cost' . $cart->id]) }}
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            <span class="pull-right">
                                @lang('theme.ship_to'):
                                <a href="javascript:void(0)" id="shipTo{{ $cart->id }}" class="ship_to"
                                    data-cart="{{ $cart->id }}" data-country="{{ $cart->ship_to_country_id }}"
                                    data-state="{{ $cart->ship_to_state_id }}">
                    ...
                    <tbody>
                        ...
                        <tr class="cart-item-tr">
                            <td>
                                ...
                                {{ Form::hidden('shipping_weight[' . $item->id . ']', $item->shipping_weight *
                                $item->pivot->quantity, ['id' => 'itemWeight' . $item->id,'class' => 'itemWeight' .
                                $cart->id]) }}

                                <img src="{{ get_product_img_src($item, 'mini') }}" class="img-mini"
                            ...
                            <td class="hidden-sm hidden-xs">
                                <div class="shopping-cart-item-title">
                                    <a href="{{ route('show.product', $item->slug) }}" class="product-info-title">
                                        {{ str_replace(',', ' - ',$item->pivot->item_description); }}
                                    </a>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="product-info-qty-item"
                                    ...
                                    <input name="quantity[{{ $item->id }}]" id="itemQtt{{ $item->id }}"
                                        class="product-info-qty product-info-qty-input" data-cart="{{ $cart->id }}"
                                        data-item="{{ $item->id }}" data-min="{{ $item->min_order_quantity }}"
                            ...
                            <td>
                                <a href="javascript:void(0)" class="cart-item-remove" data-cart="{{ $cart->id }}"
                                    data-item="{{ $item->id }}" data-toggle="tooltip"
                                    title="@lang('theme.remove_item')">&times;</a>
                            </td>
                        </tr> <!-- /.order-body -->
                        @endforeach
                    </tbody>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <div class="input-group full-width">
                                ...
                                <input name="coupon" value="{{ $cart->coupon ? $cart->coupon->code : null }}" id="coupon{{ $cart->id }}" class="form-control flat" type="text" placeholder="@lang('theme.placeholder.have_coupon_from_seller')" readonly>
        
                                <span class="input-group-btn">
                                <button class="btn btn-default flat apply_seller_coupon" type="button" data-cart="{{ $cart->id }}">@lang('theme.button.apply')</button>
                                </span>
                            </div><!-- /input-group -->
                            </td>
                        </tr>
            ...
            <div class="col-md-3 space20">
                <div class="side-widget" id="cart-summary{{ $cart->id }}">
                    ...
                    @php
                        // echo $cart_total;
                        // $gst_amount = number_format($cart_total - ($cart_total * 100)/(100 + config('app.gst_percentage')),2);
                        // $sub_total = $cart_total - $gst_amount;
                        // if($cart->discount > 0){
                        //     $gst_amount = number_format((($sub_total-$cart->discount)*10)/100,2);
                        //     $contribution_amount =  $sub_total - $cart->discount + $gst_amount;
                        // }else{
                        //     $contribution_amount =  $cart_total - $cart->discount;
                        // }
                       
                            // new calculation \
                            // $sub_total = '';
                            
                        $sub_total_1 = $cart_total - $cart->discount;
                    ...
                    <ul class="shopping-cart-summary">
                        ...
                        <li id="discount-section-li{{ $cart->id }}">
                            <span>{{ trans('theme.company_contribution') }}
                                {{-- <em id="summary-discount-name{{ $cart->id }}" class="small text-muted">{{ $cart->coupon
                                    ? $cart->coupon->name . ' (' . $cart->coupon->getFormatedAmountText() . ')' : ''
                                    }}</em> --}}
                            </span>
                            <span>{{ get_currency_prefix() }}
                            <span id="summary-discount{{ $cart->id }}">{{ $cart->coupon ?
                        ...
                        <li>
                            <span>GST</span>
                            <span>{{ get_currency_prefix() }}
                                <span id="gst_amount{{ $cart->id }}">{{$gst_amount}}</span>{{ get_currency_suffix() }}
                            </span>
                        </li>
                        ...
                        <li>
                            <span>
                                <a class="dynamic-shipping-rates" href="javascript:void(0)" data-toggle="popover"
                                    data-cart="{{ $cart->id }}" data-options="{{ $shipping_options[$cart->id] }}"
                                    id="shipping-options{{ $cart->id }}" title="{{ trans('theme.shipping') }}">
                            ...
                            <span>
                                {{-- {{ get_currency_prefix() }} --}}
                                {{-- <span id="summary-shipping{{ $cart->id }}12">{{ number_format(0, 2, '.', '') }}</span>{{
                                get_currency_suffix() }} --}}
                                Free 
                            </span>
                        </li>
                        ...
                        <li>
                            <span>
                                <a class="packaging-options" href="javascript:void(0)" data-toggle="popover"
                                    data-cart="{{ $cart->id }}" data-options="{{ $packaging_options }}"
                        ...
                        <li>
                            <span>{{ trans('theme.order_total') }}</span>
                            <span>{{ get_currency_prefix() }}
                                <span id="summary-grand-total{{ $cart->id }}total">{{ number_format($order_total, 2)
                                    }}</span>{{ get_currency_suffix() }}
                            </span>
                        </li>
                    </ul>
                ...
                @if (allow_checkout())
                <button type="submit" form="formId{{ $cart->id }}" id="checkout-btn{{ $cart->id }}"
